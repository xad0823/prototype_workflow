...
reg	[POINTER_INDEX_WIDTH-1:0] gray_wr_ptr_sync [1:SYNC_STAGE];
wire	[POINTER_INDEX_WIDTH-1:0] gray_wr_ptr_rclk = gray_wr_ptr_sync[SYNC_STAGE];
...
reg  	[POINTER_INDEX_WIDTH-1:0] gray_rd_ptr_sync [1:SYNC_STAGE];
wire  	[POINTER_INDEX_WIDTH-1:0] gray_rd_ptr_wclk = gray_rd_ptr_sync[SYNC_STAGE];
reg  	[POINTER_INDEX_WIDTH-2:0] bin_rd_ptr_w_clk;

reg		[POINTER_INDEX_WIDTH-2:0] wr_index;
reg		[POINTER_INDEX_WIDTH-2:0] rd_index_nx;
...
always @* begin:bin_wr_ptr_blk
	integer		i;
	for (i = 0; i < POINTER_INDEX_WIDTH; i = i + 1)
		bin_wr_ptr[i] = ^(gray_wr_ptr>>i);
end
...
always @(posedge w_clk or negedge w_reset_n) begin
	if (!w_reset_n) begin
		for (k=1; k <= SYNC_STAGE; k=k+1)
			gray_rd_ptr_sync[k] <= {(POINTER_INDEX_WIDTH){1'b0}};
	end
		...
		gray_rd_ptr_sync[1] <= gray_rd_ptr;
		for (k=2; k <= SYNC_STAGE; k=k+1)
			gray_rd_ptr_sync[k] <= gray_rd_ptr_sync[k-1];
	end
end
...
always @* begin: next_full_blk
	...
	for (i = POINTER_INDEX_WIDTH - 1; i >= 0; i = i - 1) begin
		if(i == POINTER_INDEX_WIDTH - 1 || i == POINTER_INDEX_WIDTH - 2)
			next_full[i] = (next_gray_wr_ptr[i] != gray_rd_ptr_wclk[i]);
		else
			next_full[i] = (next_gray_wr_ptr[i] == gray_rd_ptr_wclk[i]);
	end
end
...
always @* begin: bin_rd_ptr_w_clk_blk
	...
	for (i = 0; i < POINTER_INDEX_WIDTH - 1; i = i + 1)
		bin_rd_ptr_w_clk[i] = ^(gray_rd_ptr_wclk>>i);
end
...
always @* begin: rd_index_nx_blk
	integer		i;
	for (i = POINTER_INDEX_WIDTH - 2; i >= 0; i = i - 1) begin
		if(i == POINTER_INDEX_WIDTH - 2)
			rd_index_nx[i] = next_gray_rd_ptr[i+1] ^ next_gray_rd_ptr[i];
		else
			rd_index_nx[i] = next_gray_rd_ptr[i];
	end
end
...
wire update_rd_data = (rd | empty) & ~empty_nx;
assign rd_data_nx  = mem[rd_index_nx];
always @(posedge r_clk or negedge r_reset_n) begin
	if (!r_reset_n) begin
                rd_data <= {DATA_WIDTH{1'b0}};
        end
        else if (update_rd_data) begin
                rd_data <= rd_data_nx;
        end
end

always @* begin: bin_rd_ptr_blk
	integer		i;
	for (i = 0; i < POINTER_INDEX_WIDTH; i = i + 1)
		bin_rd_ptr[i] = ^(gray_rd_ptr>>i);
end

assign next_bin_rd_ptr =  bin_rd_ptr + {{(POINTER_INDEX_WIDTH-1){1'b0}}, rd & ~empty};
...
always @(posedge r_clk or negedge r_reset_n) begin
	if (!r_reset_n) begin
		for (j=1; j <= SYNC_STAGE; j=j+1)
			gray_wr_ptr_sync[j] <= {(POINTER_INDEX_WIDTH){1'b0}};
	end
		...
		gray_wr_ptr_sync[1] <= gray_wr_ptr;
		for (j=2; j <= SYNC_STAGE; j=j+1)
			gray_wr_ptr_sync[j] <= gray_wr_ptr_sync[j-1];
	end
end

assign empty_nx = (next_gray_rd_ptr[POINTER_INDEX_WIDTH-1:0] == gray_wr_ptr_rclk[POINTER_INDEX_WIDTH-1:0]);
always @(posedge r_clk or negedge r_reset_n) begin
	if (!r_reset_n)
		empty <= 1'b1;
	else if (r_fifo_clr)
...
always @* begin: bin_wr_ptr_r_clk_blk
	integer		i;
	for (i = 0; i < POINTER_INDEX_WIDTH; i = i + 1)
		bin_wr_ptr_r_clk[i] = ^(gray_wr_ptr_rclk>>i);
end
