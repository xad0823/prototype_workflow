...
module nic400_ib_gpio_ahb_m_ib_downsize_wr_resp_block_sse710_integration_example_f0_host_exp
  (
  bvalid_m,
  bready_s,
  bresp_m,
...
`include "nic400_ib_gpio_ahb_m_ib_defs_sse710_integration_example_f0_host_exp.v"
  ...
  input [21:0]            bchannel_data;
   ...
   assign awid = bchannel_data[21:4];
   ...
   assign next_bchannel_ready = ~(&(valid_bus | (store & {2{push_slice}}))) || (pop_slice);
   ...
   always @(match_bus)
    ...
    assign bvalid_s = (bvalid_m && match);
     ...
     nic400_ib_gpio_ahb_m_ib_downsize_resp_cam_slice_sse710_integration_example_f0_host_exp u_downsize_resp_cam_slice_0
     (
        .aresetn              (aresetn),
        .aclk                 (aclk),
     ...
     nic400_ib_gpio_ahb_m_ib_downsize_resp_cam_slice_sse710_integration_example_f0_host_exp u_downsize_resp_cam_slice_1
     (
        .aresetn              (aresetn),
        .aclk                 (aclk),
...
`include "std_ovl_defines.h"
  ...
  assert_zero_one_hot #(`OVL_FATAL,2,0,"More than one B Channel Slice matched")
  ovl_single_ID_match
    (
     .clk              (aclk),
     .reset_n          (aresetn),
     .test_expr        ({2{bvalid_m}} & match_bus)
     );


  assert_zero_one_hot #(`OVL_FATAL,2,0,"More than one B Channel Slice hazard")
  ovl_single_hazard
    (
     .clk              (aclk),
     .reset_n          (aresetn),
     .test_expr        ({2{bchannel_valid}} & hazard_bus)
     );
  ...
  ovl_input_hndshk_error
   ...
   assert_fifo_index #(`OVL_FATAL, 2, 1, 1, 0,"ERROR, BChannel Look-Up under or overflowed")
