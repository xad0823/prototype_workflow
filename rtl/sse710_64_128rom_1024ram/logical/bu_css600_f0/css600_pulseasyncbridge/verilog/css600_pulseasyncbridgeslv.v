...
module css600_pulseasyncbridgeslv #(parameter
  WIDTH         = 1,
  WAKE_ON_PULSE = 1,
  FF_SYNC_DEPTH = 2
)
(
  ...
  wire [WIDTH_INT-1:0] pulse_ack_sync;
  ...
  assign pulse_req_next = (pulse_in_pulse & ~pulse_ack_sync) |
                          (pulse_req_q    & ~pulse_ack_sync);
  ...
  css600_pulseasyncbridgeslv_async #(.NUM_OR_INPUTS(2*WIDTH_INT))
    u_qactive_async (
      .or_inputs     (or_inputs),
  ...
  generate
    for(i = 0; i < WIDTH_INT; i = i + 1) begin: gen_ack_sync_no_cdc
      css600_cdc_capt_sync
      #(.FF_SYNC_DEPTH (FF_SYNC_DEPTH_INT))
      u_css600_cdc_capt_sync_ack(
        .clk       (clk_s),
        .reset_n   (reset_s_n),
        .d_async_i (pulse_ack[i]),
        .q_sync_o  (pulse_ack_sync[i])
      );
    end
  endgenerate
