...
(
  ...
  output wire   [6:0] mstr_prot,
  output wire   [2:0] mstr_size,
  output wire         mstr_nonsec,
...
`include "css600_ahbap_localparams.v"
  ...
  wire [31:0]        next_ap0_tar;
  wire [31:0]        next_ap1_tar;
  wire               ap0_log_err;
  wire               ap1_log_err;
  ...
  wire [1:0]         next_csw_size;
  wire [4:0]         next_csw_prot;
  wire               next_csw_prot6;
  reg                boss_twin;
  reg                penable_reconst;
  ...
  assign secure_mem_req = paddr_s[12] ? ~ap1_csw_nonsec : ~ap0_csw_nonsec;
  ...
  assign mem_rd_en = (curr_state == ST_APBSLV_MEM) &
                     (next_state == ST_APBSLV_RDY) & ~pwrite_s;
  ...
  assign next_csw_size = |(pwdata_s[2:1]) ? 2'b10 : pwdata_s[1:0];

   assign {next_csw_prot6,next_csw_prot} = ( ( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b0_000)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b0_001)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b0_010)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b0_110)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b0_111)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b1_010)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b1_110)
                                           ||( {pwdata_s[15],pwdata_s[28:24+2]} == 4'b1_111)
                                           ) ? {pwdata_s[15],pwdata_s[28:24  ]}
                                             : {AHBM_HPROT6_DEFAULT,AHBM_HPROT_DEFAULT}
                                             ;
  ...
  begin : p_ap0_csw
    ...
    begin
      ...
      ap0_csw_prot6    <= AHBM_HPROT6_DEFAULT;
      ap0_csw_errnpass <= 1'b0;
      ap0_csw_errstop  <= 1'b0;
      ap0_csw_prot     <= AHBM_HPROT_DEFAULT;
      ap0_csw_nonsec   <= 1'b1;
    end
    else
      if (ap0_csw_sw_wr_en)
      begin
        ap0_csw_size     <= next_csw_size;
        ap0_csw_addr_inc <= pwdata_s[4];
        ap0_csw_prot6    <= next_csw_prot6;
        ap0_csw_errnpass <= pwdata_s[16];
        ap0_csw_errstop  <= pwdata_s[17];
        ap0_csw_prot     <= next_csw_prot;
        ap0_csw_nonsec   <= pwdata_s[30];
      end
  end
  ...
  begin : p_ap1_csw
    ...
    begin
      ...
      ap1_csw_prot6    <= AHBM_HPROT6_DEFAULT;
      ap1_csw_errnpass <= 1'b0;
      ap1_csw_errstop  <= 1'b0;
      ap1_csw_prot     <= AHBM_HPROT_DEFAULT;
      ap1_csw_nonsec   <= 1'b1;
    end
    else
      if (ap1_csw_sw_wr_en)
      begin
        ap1_csw_size     <= next_csw_size;
        ap1_csw_addr_inc <= pwdata_s[4];
        ap1_csw_prot6    <= next_csw_prot6;
        ap1_csw_errnpass <= pwdata_s[16];
        ap1_csw_errstop  <= pwdata_s[17];
        ap1_csw_prot     <= next_csw_prot;
        ap1_csw_nonsec   <= pwdata_s[30];
      end
  end
  ...
  always @ (*) begin : p_addr_inc0
    case (ap0_csw_size)
      AHBAP_BYTE      : ap0_addr_inc_size = 4'b0001;
      AHBAP_HALF_WORD : ap0_addr_inc_size = 4'b0010;
      AHBAP_WORD      : ap0_addr_inc_size = 4'b0100;
      AHBAP_DWORD     : ap0_addr_inc_size = 4'b1000;
      default         : ap0_addr_inc_size = 4'b0000;
    endcase
  end
  ...
  always @ (*) begin : p_addr_inc1
    case (ap1_csw_size)
      AHBAP_BYTE      : ap1_addr_inc_size = 4'b0001;
      AHBAP_HALF_WORD : ap1_addr_inc_size = 4'b0010;
      AHBAP_WORD      : ap1_addr_inc_size = 4'b0100;
      AHBAP_DWORD     : ap1_addr_inc_size = 4'b1000;
      default         : ap1_addr_inc_size = 4'b0000;
    endcase
  end
  ...
  assign apb_rdata_en = (ap_select & ~mem_access & ~pwrite_s)
                      |  mem_rd_en
                      | (mem_access  & ~pwrite_s & (next_state == ST_APBSLV_ERR));

  always @(posedge clk or negedge reset_n)
  begin : p_prdata_s
    if (!reset_n)
      prdata_s <= 32'b0;
    else if (apb_rdata_en)
  ...
  assign apb_rd_en = next_apbslv_ready
  ...
  always @*
  begin
    mstr_addr[31:10] = (boss_twin == LOGICAL_AP1)
                      ? ap1_tar[31:10]
                      : ap0_tar[31:10];
    mstr_addr[1:0] = 2'b00;
    case({ap0_dar_access, ap1_dar_access, ap0_bd_access, ap1_bd_access})
      4'b0000  :
        begin
          case (sel_mstr_size)
             AHBAP_BYTE      : mstr_addr[9:0] = (boss_twin == LOGICAL_AP1)
                                                ? ap1_tar[9:0]
                                                : ap0_tar[9:0];
             AHBAP_HALF_WORD : mstr_addr[9:0] = (boss_twin == LOGICAL_AP1)
                                                ? {ap1_tar[9:1], 1'b0}
                                                : {ap0_tar[9:1], 1'b0};
             AHBAP_WORD      : mstr_addr[9:0] = (boss_twin == LOGICAL_AP1)
                                                ? {ap1_tar[9:2], 2'b00}
                                                : {ap0_tar[9:2], 2'b00};
             AHBAP_DWORD     : mstr_addr[9:0] = (boss_twin == LOGICAL_AP1)
                                                ? {ap1_tar[9:3], 3'b000}
                                                : {ap0_tar[9:3], 3'b000};
  ...
  assign mstr_prot[4:0] = (boss_twin == LOGICAL_AP1) ? ap1_csw_prot  : ap0_csw_prot ;
  assign mstr_prot[5]   = 1'b0;
  assign mstr_prot[6]   = (boss_twin == LOGICAL_AP1) ? ap1_csw_prot6 : ap0_csw_prot6;

  assign sel_mstr_size = (boss_twin == LOGICAL_AP1) ? ap1_csw_size : ap0_csw_size;

  assign mstr_size = {1'b0, sel_mstr_size};

  assign mstr_nonsec = (boss_twin == LOGICAL_AP1) ? ap1_csw_nonsec : ap0_csw_nonsec;
