//----------------------------------------------------------------------------
//   The confidential and proprietary information contained in this file may
//   only be used by a person authorised under and to the extent permitted
//   by a subsisting licensing agreement from Arm Limited or its affiliates.
//
//          (C) COPYRIGHT 2016-2017, 2019 Arm Limited or its affiliates.
//              ALL RIGHTS RESERVED
//
//   This entire notice must be reproduced on all copies of this file
//   and copies of this file may only be made by a person if such person is
//   permitted to do so under the terms of a subsisting license agreement
//   from Arm Limited or its affiliates.
//----------------------------------------------------------------------------
//   Release information : TM200-MN-22110-r4p1-00rel0
//
//----------------------------------------------------------------------------
//   Description :
//   Top level of css600_apbap
//
//----------------------------------------------------------------------------


module css600_apbap #( parameter
  FF_SYNC_DEPTH = 2,
  REVAND        = 4'h0
)
(
  clk,
  reset_n,
  pwakeup_s,
  psel_s,
  penable_s,
  pwrite_s,
  paddr_s,
  pwdata_s,
  pready_s,
  pslverr_s,
  prdata_s,
  pwakeup_m,
  psel_m,
  penable_m,
  pwrite_m,
  paddr_m,
  pwdata_m,
  pprot_m,
  pready_m,
  pslverr_m,
  prdata_m,
  clk_qreq_n,
  clk_qaccept_n,
  clk_qdeny,
  clk_qactive,
  ap_en,
  ap_secure_en,
  dp_abort,
  dftcgen,
  baseaddr,
  baseaddr_valid
);


  input wire         clk;
  input wire         reset_n;

  input wire         pwakeup_s;
  input wire         psel_s;
  input wire         penable_s;
  input wire         pwrite_s;
  input wire  [12:0] paddr_s;
  input wire  [31:0] pwdata_s;
  output wire        pready_s;
  output wire        pslverr_s;
  output wire [31:0] prdata_s;

  output wire        pwakeup_m;
  output wire        psel_m;
  output wire        penable_m;
  output wire        pwrite_m;
  output wire [31:0] paddr_m;
  output wire [31:0] pwdata_m;
  output wire [2:0]  pprot_m;
  input wire         pready_m;
  input wire         pslverr_m;
  input wire  [31:0] prdata_m;

  input wire         clk_qreq_n;
  output wire        clk_qaccept_n;
  output wire        clk_qdeny;
  output wire        clk_qactive;

  input wire         ap_en;
  input wire         ap_secure_en;

  input wire         dp_abort;

  input wire         dftcgen;

  input wire  [31:0] baseaddr;
  input wire         baseaddr_valid;

  localparam INT_FF_SYNC_DEPTH = (FF_SYNC_DEPTH == 3) ? 3 : 2;

  wire                  mstr_tr_req;
  wire                  mstr_rd_wr;
  wire           [31:0] mstr_addr;
  wire            [2:0] mstr_prot;
  wire                  mstr_tr_done;
  wire           [31:0] mstr_wr_data;
  wire           [31:0] mstr_rd_data;
  wire                  mstr_err;

  wire                  clk_g;
  wire                  int_clk_en /* verilator clock_enable */;
  wire                  dev_active;
  wire                  lp_request;
  wire                  lp_done;
  wire                  mstr_tr_in_prog;
  wire                  dev_run;
  wire                  itctrl_ime;
  wire                  clk_qreq_n_sync;

  wire            [3:0] w_revand;


  css600_apbap_apbslv
    u_css600_apbap_apbslv
    (
     .clk               (clk_g),
     .reset_n           (reset_n),
     .psel_s            (psel_s),
     .penable_s         (penable_s),
     .pwrite_s          (pwrite_s),
     .paddr_s           (paddr_s[12:0]),
     .pwdata_s          (pwdata_s[31:0]),
     .pready_s          (pready_s),
     .pslverr_s         (pslverr_s),
     .prdata_s          (prdata_s[31:0]),
     .ap_en             (ap_en),
     .ap_secure_en      (ap_secure_en),
     .dp_abort          (dp_abort),
     .dev_run           (dev_run),
     .itctrl_ime        (itctrl_ime),
     .baseaddr          (baseaddr[31:0]),
     .baseaddr_valid    (baseaddr_valid),
     .mstr_tr_req       (mstr_tr_req),
     .mstr_rd_wr        (mstr_rd_wr),
     .mstr_addr         (mstr_addr[31:0]),
     .mstr_prot         (mstr_prot[2:0]),
     .mstr_tr_in_prog   (mstr_tr_in_prog),
     .mstr_tr_done      (mstr_tr_done),
     .mstr_rd_data      (mstr_rd_data[31:0]),
     .mstr_err          (mstr_err),
     .revand            (w_revand)
    );

  assign mstr_wr_data = pwdata_s[31:0];

  assign paddr_m = mstr_addr[31:0];

  assign pprot_m = mstr_prot[2:0];


  css600_apbap_apbmstr
    u_css600_apbap_apbmstr
    (
     .clk          (clk_g),
     .reset_n      (reset_n),
     .psel_m       (psel_m),
     .penable_m    (penable_m),
     .pwrite_m     (pwrite_m),
     .pwdata_m     (pwdata_m[31:0]),
     .pready_m     (pready_m),
     .mstr_tr_req  (mstr_tr_req),
     .mstr_rd_wr   (mstr_rd_wr),
     .mstr_wr_data (mstr_wr_data[31:0]),
     .mstr_tr_done (mstr_tr_done)
    );


  assign mstr_tr_in_prog = psel_m;
  assign mstr_rd_data = prdata_m[31:0];
  assign mstr_err = pslverr_m;

  assign pwakeup_m = psel_m;


  css600_clk_gate
    u_css600_clk_gate
    (
     .clk_i         (clk),
     .clk_enable_i  (int_clk_en),
     .clk_o         (clk_g),
     .dftcgen       (dftcgen)
    );

  assign int_clk_en = dev_active & dev_run;


    css600_cdc_capt_sync
      #(.FF_SYNC_DEPTH (INT_FF_SYNC_DEPTH))
    u_css600_cdc_capt_sync_qreq(
                                .clk       (clk),
                                .reset_n   (reset_n),
                                .d_async_i (clk_qreq_n),
                                .q_sync_o  (clk_qreq_n_sync)
                               );

  css600_lpislave
    u_css600_lpislave
    (
      .clk          (clk),
      .reset_n      (reset_n),
      .qreq_sync_n  (clk_qreq_n_sync),
      .qaccept_n    (clk_qaccept_n),
      .qdeny        (clk_qdeny),
      .lp_request   (lp_request),
      .dev_active   (dev_active),
      .lp_done      (lp_done),
      .dev_run      (dev_run),
      .cg_en        ()
    );

  assign dev_active = (psel_s | mstr_tr_in_prog | itctrl_ime);

  assign lp_done = lp_request;

  css600_apbap_async u_qactive_async (
    .clk_qactive (clk_qactive),
    .pwakeup_s (pwakeup_s),
    .mstr_tr_in_prog (mstr_tr_in_prog),
    .itctrl_ime (itctrl_ime)
  );

  css600_ecorevnum #(.WIDTH(4), .ECOREVVAL(REVAND))
    u_css600_ecorevnum (.ecorevnum(w_revand));


endmodule

