...
module nic400_asib_debug_axis_wr_spi_cdas_sse710_main
  (
    aw_enable,
    wr_enable,
    asel,
  ...
  wire  [26:0]    mask;   
  wire  [26:0]    id_match;   
  wire  [26:0]    dest_match;   
  wire  [26:0]    resp_match;   
  wire           cds_stall;    
  wire           t_push;     
  ...
  reg            aready_reg;   
  reg   [4:0]    tt_cnt;   
  reg   [9:0]    spi_asel [26:0];   
  reg   [3:0]    spi_ids [26:0];   
  reg   [26:0]    spi_valid;   
  reg   [9:0]    hold_asel;   
  reg   [3:0]    hold_aid;   
  reg            hold_valid;   
  reg            t_ready;   
   ...
   assign  mask[26:24] = {3{|(free_slot0|free_slot1|free_slot2|free_slot3|free_slot4|free_slot5)}}; 

   assign free_slot = {free_slot6,
                       free_slot5,
                       free_slot4,
                       free_slot3,
                       free_slot2,
   ...
   assign resp_match[0] = ((spi_ids[0]  == resp_id[3:0]) &&
                            spi_valid[0]);
   assign resp_match[1] = ((spi_ids[1]  == resp_id[3:0]) &&
                            spi_valid[1]);
   assign resp_match[2] = ((spi_ids[2]  == resp_id[3:0]) &&
                            spi_valid[2]);
   assign resp_match[3] = ((spi_ids[3]  == resp_id[3:0]) &&
                            spi_valid[3]);
   assign resp_match[4] = ((spi_ids[4]  == resp_id[3:0]) &&
                            spi_valid[4]);
   assign resp_match[5] = ((spi_ids[5]  == resp_id[3:0]) &&
                            spi_valid[5]);
   assign resp_match[6] = ((spi_ids[6]  == resp_id[3:0]) &&
                            spi_valid[6]);
   assign resp_match[7] = ((spi_ids[7]  == resp_id[3:0]) &&
                            spi_valid[7]);
   assign resp_match[8] = ((spi_ids[8]  == resp_id[3:0]) &&
                            spi_valid[8]);
   assign resp_match[9] = ((spi_ids[9]  == resp_id[3:0]) &&
                            spi_valid[9]);
   assign resp_match[10] = ((spi_ids[10]  == resp_id[3:0]) &&
                            spi_valid[10]);
   assign resp_match[11] = ((spi_ids[11]  == resp_id[3:0]) &&
                            spi_valid[11]);
   assign resp_match[12] = ((spi_ids[12]  == resp_id[3:0]) &&
                            spi_valid[12]);
   assign resp_match[13] = ((spi_ids[13]  == resp_id[3:0]) &&
                            spi_valid[13]);
   assign resp_match[14] = ((spi_ids[14]  == resp_id[3:0]) &&
                            spi_valid[14]);
   assign resp_match[15] = ((spi_ids[15]  == resp_id[3:0]) &&
                            spi_valid[15]);
   assign resp_match[16] = ((spi_ids[16]  == resp_id[3:0]) &&
                            spi_valid[16]);
   assign resp_match[17] = ((spi_ids[17]  == resp_id[3:0]) &&
                            spi_valid[17]);
   assign resp_match[18] = ((spi_ids[18]  == resp_id[3:0]) &&
                            spi_valid[18]);
   assign resp_match[19] = ((spi_ids[19]  == resp_id[3:0]) &&
                            spi_valid[19]);
   assign resp_match[20] = ((spi_ids[20]  == resp_id[3:0]) &&
                            spi_valid[20]);
   assign resp_match[21] = ((spi_ids[21]  == resp_id[3:0]) &&
                            spi_valid[21]);
   assign resp_match[22] = ((spi_ids[22]  == resp_id[3:0]) &&
                            spi_valid[22]);
   assign resp_match[23] = ((spi_ids[23]  == resp_id[3:0]) &&
                            spi_valid[23]);
   assign resp_match[24] = ((spi_ids[24]  == resp_id[3:0]) &&
                            spi_valid[24]);
   assign resp_match[25] = ((spi_ids[25]  == resp_id[3:0]) &&
                            spi_valid[25]);
   assign resp_match[26] = ((spi_ids[26]  == resp_id[3:0]) &&
                            spi_valid[26]);
   ...
   assign  resp_mask[26:24] = {3{|(resp_sel0|resp_sel1|resp_sel2|resp_sel3|resp_sel4|resp_sel5)}}; 

   assign resp_sel = {resp_sel6,
                       resp_sel5,
                       resp_sel4,
                       resp_sel3,
                       resp_sel2,
   ...
   assign hold_resp_match = (hold_aid == resp_id[3:0]) & (~|resp_sel)
                            & hold_valid & resp_valid & resp_ready;

   assign t_push = t_ready & ~hold_resp_match & hold_valid;

   assign id_match[0] = (spi_ids[0] == aid[3:0]);
   assign id_match[1] = (spi_ids[1] == aid[3:0]);
   assign id_match[2] = (spi_ids[2] == aid[3:0]);
   assign id_match[3] = (spi_ids[3] == aid[3:0]);
   assign id_match[4] = (spi_ids[4] == aid[3:0]);
   assign id_match[5] = (spi_ids[5] == aid[3:0]);
   assign id_match[6] = (spi_ids[6] == aid[3:0]);
   assign id_match[7] = (spi_ids[7] == aid[3:0]);
   assign id_match[8] = (spi_ids[8] == aid[3:0]);
   assign id_match[9] = (spi_ids[9] == aid[3:0]);
   assign id_match[10] = (spi_ids[10] == aid[3:0]);
   assign id_match[11] = (spi_ids[11] == aid[3:0]);
   assign id_match[12] = (spi_ids[12] == aid[3:0]);
   assign id_match[13] = (spi_ids[13] == aid[3:0]);
   assign id_match[14] = (spi_ids[14] == aid[3:0]);
   assign id_match[15] = (spi_ids[15] == aid[3:0]);
   assign id_match[16] = (spi_ids[16] == aid[3:0]);
   assign id_match[17] = (spi_ids[17] == aid[3:0]);
   assign id_match[18] = (spi_ids[18] == aid[3:0]);
   assign id_match[19] = (spi_ids[19] == aid[3:0]);
   assign id_match[20] = (spi_ids[20] == aid[3:0]);
   assign id_match[21] = (spi_ids[21] == aid[3:0]);
   assign id_match[22] = (spi_ids[22] == aid[3:0]);
   assign id_match[23] = (spi_ids[23] == aid[3:0]);
   assign id_match[24] = (spi_ids[24] == aid[3:0]);
   assign id_match[25] = (spi_ids[25] == aid[3:0]);
   assign id_match[26] = (spi_ids[26] == aid[3:0]);
   ...
   assign cds_stall = |(id_match & ~dest_match & spi_valid) |
                      ((hold_aid == aid[3:0]) & (hold_asel != asel) & hold_valid);
   ...
   always @(posedge aclk or negedge aresetn)
    ...
    begin : p_ptr_seq
      ...
      begin
        if (t_push)
         begin
             for (tt_ptr_val = 0; tt_ptr_val<27; tt_ptr_val=tt_ptr_val+1)
   ...
   always @(posedge aclk)
    begin : p_tt_dat_seq
         ...
         if (t_push)
          begin
                for (tt_ptr = 0; tt_ptr<27; tt_ptr=tt_ptr+1)
