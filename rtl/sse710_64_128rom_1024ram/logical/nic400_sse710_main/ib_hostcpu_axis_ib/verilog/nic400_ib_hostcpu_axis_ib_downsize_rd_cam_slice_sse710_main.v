...
module nic400_ib_hostcpu_axis_ib_downsize_rd_cam_slice_sse710_main
  (
    aresetn,
    aclk,
...
`include "Axi.v"
  ...
  input [4:0]            hazard_pointer;
  input [4:0]            match_pointer;
  input                  hazard_in;

  input [14:0]           archannel_data;
  input [16:0]           ar_cam_data;            
  ...
  output [2:0]           beat_number;
  
  output [63:0]          rdata_cam;
  ...
  reg [3:0]             addr_next;
  reg [3:0]             addr_reg;
  reg [3:0]             addr_mask_reg;
  reg [2:0]             addr_mask_end_reg;
  reg [2:0]             size_reg;
  reg                   bypass_reg;
  reg [3:0]             downsize_reg;
  
  reg [4:0]             size_incr;
  
  reg [2:0]             beats_masked;
  reg [2:0]             addr_end_masked;
  ...
  wire [7:0]            next_id_reg;
  wire [4:0]            next_pointer_reg;

  wire                  next_hazard_reg;
  wire                  next_valid_reg;
  wire                  next_last_reg;
  wire [3:0]               next_n_response_reg;
  wire [3:0]            next_addr_reg;
  wire [3:0]            next_addr_mask_reg;
  ...
  wire                  rdata_reg_en;
  wire [63:0]           next_rdata_reg;

  wire                  last_addr_match_i;
  wire                  match_i;

 assign addr_mask     = archannel_data[3:0];
 assign addr_mask_end = archannel_data[6:4];
 assign n_response    = fixed_mask ? {4{1'b0}} : archannel_data[10:7];
 assign downsize      = archannel_data[14:11];

 assign bypass_in     = ar_cam_data[0];
 assign fixed_mask    = ar_cam_data[1];
 assign size_in       = ar_cam_data[4:2];
 assign addr          = ar_cam_data[8:5];
 assign hazard_id     = ar_cam_data[16:9];
 ...
 assign can_complete     = match_i && ~|n_response_reg;
  ...
  assign data_select = ~addr_out[3];
 
 assign rdata_reg_en   = ((data_select) && downsize_reg[3] && match_i) | clear_slice;
 assign next_rdata_reg = clear_slice ? 64'b0 : (update && match_i) ? rdata_m : rdata_reg;
 ...
 assign rdata_cam[63:0] = (rdata_cam_valid ? rdata_reg : rdata_m) & {64{match_i}};
 ...
 always @(posedge aclk or negedge aresetn)
  ...
  assign rdata_cam_valid = downsize_reg[3] & addr_out[3];
 ...
 assign valid_reg_wr_en = default_reg_wr_en || (match_i && pop_slice);
  ...
  begin : beats_masked_p
    ...
    case (downsize_reg)
       4'd0    : beats_masked = (~addr_out[2:0]) & (addr_mask_reg[2:0]);
       4'd1    : beats_masked = ({{1{1'b0}}, ~addr_out[2:1]}) & (addr_mask_reg[2:0]);          
       4'd3    : beats_masked = ({{2{1'b0}}, ~addr_out[2]}) & (addr_mask_reg[2:0]);          
  ...
  begin : addr_end_masked_p
    ...
    addr_end_masked = {3{1'b0}};
    
    case (downsize_reg)
       4'd0    : addr_end_masked = (addr_mask_end_reg[2:0]);
       4'd1    : addr_end_masked = {{1{1'b0}}, addr_mask_end_reg[2:1]};          
       4'd3    : addr_end_masked = {{2{1'b0}}, addr_mask_end_reg[2]};          
  ...
  begin : size_incr_p
    case (size_reg)
       3'd0    : size_incr = {4'b0, 1'b1}; 
       3'd1    : size_incr = {3'b0, 2'b10};  
       3'd2    : size_incr = {2'b0, 3'b100};  
       3'd3    : size_incr = {1'b0, 4'b1000};  
       3'd4,  
       3'd5,  
       3'd6,  
       3'd7    : size_incr = {4'b0, 1'b1};    
       default         : size_incr = {5'bx};
    endcase
  end
  ...
  begin : addr_reg_nxt_p
     integer index_i;
     for (index_i = 0; index_i < 4 ; index_i = index_i + 1)
     begin
  ...
  assign addr_out = addr_reg & {4{match_i}};
  assign last_addr_match_i = (~|n_response_reg || (downsize_reg[3] & (~|addr_mask_reg)));
  assign last_addr_match   = last_addr_match_i;
  assign beat_complete     = boundary_cross;
  
  assign beat_number = bypass_reg ? {3{1'b0}} : 
                        ((last_addr_match_i && rlast_m) ? (addr_end_masked & {3{match_i}}) : 
                          ((beats_masked) & {3{match_i}}));
  ...
  assign next_addr_mask_reg = (fixed_incr_addr) ? {4{1'b1}} : addr_mask;
...
`include "std_ovl_defines.h"
  ...
  assign      size_out_range = (size_reg > 3'd3);


  wire                    illegal_size_incr;        

  assign illegal_size_incr =  (size_incr == {4'b0, 1'b1}) & size_out_range;
  ...
  (
    ...
    .reset_n    (aresetn),
    .qualifier  (valid_reg),
    .test_expr  (rdata_buffer_enables & {7{nic400_ib_hostcpu_axis_ib_downsize_rd_chan_sse710_main.rvalid_m}})
  );
`endif
