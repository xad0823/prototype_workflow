...
module nic400_ib_hostcpu_axis_ib_downsize_rd_addr_fmt_sse710_main
  (
    aresetn,
    aclk,
...
`include "Axi.v"
  ...
  wire [4:0]                boundary_incr;wire                      overflow_reg;
 ...
 always @(*)
  begin : p_end_addr_mask
    ...
    read_mask_end = {3{1'b1}};
    
    case (arburst_s)
      `AXI_ABURST_FIXED : read_mask_end = {3{1'b1}};
      `AXI_ABURST_WRAP  : read_mask_end =  wrap_fits ? total_bytes[2:0] : addr_less_one[2:0];
      `AXI_ABURST_INCR  : read_mask_end = (arlen_m == `AXI_ALEN_1) ? total_bytes[2:0] : final_address[2:0];
      default           : read_mask_end = {3{1'bx}};
    endcase
  end 
 ...
 always @(*)
  begin : p_new_addr_incr_en_r
    ...
    case (arburst_s)
      `AXI_ABURST_FIXED : read_mask = {4{1'b0}};
      `AXI_ABURST_WRAP  : read_mask = total_bytes[3:0];
      `AXI_ABURST_INCR  : read_mask = {4{1'b1}};
      default           : read_mask = {4{1'bx}};
    endcase
  end 
 ...
 assign addr_less_one = araddr_s[11:0] - {{11{1'b0}},1'b1};
 ...
 assign araddr_aligned = {araddr_s[39:12],((araddr_s[11:0]) & (~total_bytes))};
 ...
 always @(posedge aclk or negedge aresetn)
    begin : sticky_valid_reg
      if (!aresetn) begin
          ...
          long_burst_int         <= 1'b0;
      end else if(!arvalid_s | (arvalid_s & arready_s)) begin
 ...
 assign bypass = (arburst_s == `AXI_ABURST_FIXED && (align_mask)) || ( (align_mask) ? ((~arcache_s[1]) || (arlen_s == `AXI_ALEN_1) || (bypass_merge_int) ) : 1'b0 );
 
 assign no_long_bursts = ~long_burst_int & ~(|arlen_s[7:4]);
 ...
 assign  wrap_is_incr = ~|(araddr_s[11:0] & total_bytes);
 ...
 assign bytes_to_transfer_wrap1 = ~(araddr_s[11:0]) & wrap_boundary_mask;
 ...
 always @(*)
  begin : total_bytes_masked_p
    case (arsize_s)
       `AXI_ASIZE_8    : total_bytes_masked = {4'b0, arlen_s};
       `AXI_ASIZE_16   : total_bytes_masked = {3'b0, arlen_s, ~araddr_s[0]};
       `AXI_ASIZE_32   : total_bytes_masked = {2'b0, arlen_s, ~araddr_s[1:0]};
       `AXI_ASIZE_64   : total_bytes_masked = {1'b0, arlen_s, ~araddr_s[2:0]};
       `AXI_ASIZE_128  : total_bytes_masked = {arlen_s, ~araddr_s[3:0]};
       `AXI_ASIZE_256,
       `AXI_ASIZE_512,
       `AXI_ASIZE_1024 : total_bytes_masked = {4'b0, arlen_s};    
  ...
  always @(*)
   begin : total_bytes_fixed_p
      case (arsize_s)
        ...
        `AXI_ASIZE_16   : total_bytes_fixed = {{11{1'b0}}, ~araddr_s[0]};
        `AXI_ASIZE_32   : total_bytes_fixed = {{10{1'b0}}, ~araddr_s[1:0]};
        `AXI_ASIZE_64   : total_bytes_fixed = {{9{1'b0}}, ~araddr_s[2:0]};
        `AXI_ASIZE_128  : total_bytes_fixed = {{8{1'b0}}, ~araddr_s[3:0]};
        `AXI_ASIZE_256,
       `AXI_ASIZE_512,
 ...
 assign offset_address = (wrap_split_reg) ? araddr_aligned[2:0] : 
                            ((trans_overflow_reg) ? araddr_incr[2:0] : 
                            araddr_s[2:0]);
 ...
 always @(posedge aclk or negedge aresetn)
  ...
  assign base_address = (wrap_split_reg) ? araddr_s[11:0] &  ~wrap_boundary_mask : araddr_s[11:0];
  ...
  assign bytes_in_transfer_aligned = (trans_overflow & ~trans_overflow_reg) ? 
                  bytes_in_transfer & {{9{1'b1}}, ~araddr_s[2:0]} : 
                  bytes_in_transfer;
 ...
 always @(posedge aclk or negedge aresetn)
  ...
  assign wrap_overflow = (arburst_s == `AXI_ABURST_WRAP) ?
                          arvalid_s & (|total_bytes[11:7]) : 1'b0;


  
  assign boundary_incr = araddr_aligned[11:7] + 5'b00001;

  assign araddr_boundary = ({araddr_aligned[39:12],boundary_incr,araddr_aligned[6:0]});
  ...
  assert_never #(0,0,"ERROR, Transaction incoming that has arsize too large for incoming bus")
      ovl_max_input_size
       (
        .clk       (aclk),
        .reset_n   (aresetn),
        .test_expr (arvalid_s && (arsize_s > 4))
       );
