...
module nic400_ib_hostcpu_axis_ib_downsize_wr_mux_sse710_main
  (
    wdata_m,
    wstrb_m,
    wlast_m,
  ...
  assign stall_reg_en  = awvalid_m_i || stall;
   ...
   assign awaddr_in = awfifo_hndshk ? awaddr_s[3:0] : addr_reg;
   ...
   assign data_select = addr_internal[3:3];

   always @(data_select or wdata_s)
     begin : p_wdata_mux
       case (data_select)
         1'd0 : wdata_m = wdata_s[63:0];
         1'd1 : wdata_m = wdata_s[127:64];
         default : wdata_m = 64'bx;
       endcase
     end 
   ...
   always @(data_select or wstrb_s)
     begin : p_wstrb_mux
       case (data_select)
         1'd0 : wstrb_m = wstrb_s[7:0];
         1'd1 : wstrb_m = wstrb_s[15:8];
         default : wstrb_m = 8'bx;
       endcase
     end 
   ...
   always @(awlen_in or awsize_in)
     begin : p_total_bytes
       case (awsize_in)
         3'd0   : total_bytes = awlen_in[6:0];         
         3'd1   : total_bytes = {awlen_in[5:0], 1'b1};    
         3'd2   : total_bytes = {awlen_in[4:0], 2'b11};     
         3'd3   : total_bytes = {awlen_in[3:0], 3'b111};           
         3'd4,  
       3'd5,  
       3'd6,  
       3'd7   : total_bytes = awlen_in[6:0]; 
         
         default         : total_bytes = {7'bx};
       endcase
     end
   ...
   always @( awburst_in or total_bytes )
     begin : p_new_addr_incr_en
       case (awburst_in)
         `AXI_ABURST_FIXED : addr_mask = {4{1'b0}};
         `AXI_ABURST_WRAP  : addr_mask = total_bytes[3:0];
         `AXI_ABURST_INCR  : addr_mask = {4{1'b1}};
         default           : addr_mask = {4{1'bx}};
       endcase
     end 
   ...
   assign cross_boundary = |((addr_incr ^ {1'b0, awaddr_in}) & ({1'b1, ~addr_mask} | {1'b0,~downsize_in,3'b0}));   
...
`include "std_ovl_defines.h"
  ...
  assign      awsize_out_range = (awsize_in > 3'd3);
  ...
  assign illegal_total_bytes =  (total_bytes == {3'b0, awlen_in}) & awsize_out_range;
  ...
  assign illegal_size_incr =  (size_incr == {4'b0, 1'b1}) & awsize_out_range;
