...
module nic400_ib_hostcpu_axis_ib_downsize_wr_cntrl_sse710_main
  (
    aresetn,
    aclk,
...
`include "Axi.v"
  ...
  input [127:0]             wdata_merged;    
  input [15:0]              wstrb_merged;    
  
  output                    wvalid_m;     
  input                     wready_m;     
  
  output [127:0]            wdata_m;
  output [15:0]             wstrb_m;
  output                    wlast_m;
  ...
  wire                      reg_wr_en;              
  wire                      reg_update;
  wire [4:0]                downsize_boundary_mask;
 ...
 begin : size_incr_p
    case (a_size)
       ...
       `AXI_ASIZE_128  : size_incr = {5'b10000};     
       `AXI_ASIZE_256,
       `AXI_ASIZE_512,
       `AXI_ASIZE_1024 : size_incr = {4'b0, 1'b1};    
 ...
 assign downsize_boundary_mask = {{1{1'b0}},1'b1,{3{1'b0}}};
...
`include "std_ovl_defines.h"
     ...
     assign      asize_out_range = (a_size > `AXI_ASIZE_128);
                                                                                             
     wire        illegal_size_incr;        

     assign illegal_size_incr =  (size_incr == {4'b0, 1'b1}) & asize_out_range;
