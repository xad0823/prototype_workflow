...
module nic400_ib_hostcpu_axis_ib_downsize_wr_addr_fmt_sse710_main
  (
    aresetn,
    aclk,
...
`include "Axi.v"
  ...
  output [11:0]            bdata_data;
  output                   bdata_valid;
  input                    bdata_ready;

  input                    awfmt_ready;
  output                   awfmt_valid;
  output [12:0]            awfmt_data;
  ...
  reg [11:0]                 wrap_bytes_remaining_reg; 
  reg [7:0]                 awaddr_reg;
  ...
  wire [7:0]                inc_addr;
  wire [39:0]               awaddr_incr;
  wire [11:0]                next_incr_bytes_remaining_reg;
  wire                      incr_info_write_en;
  reg [3:0]                  aw_count_reg;
  reg [11:0]                 incr_bytes_remaining_reg;  
  reg [11:0]                addr_mask;
  
  reg                       fixed_overflow_reg;
  reg [11:0]                 total_bytes_fixed;  
  ...
  wire [39:0]               awaddr_aligned;
  wire [39:0]               awaddr_boundary;                                                  

  wire [6:0]                total_bytes_ext;      

  wire [11:0]                wrap_boundary_mask;   

  wire                      wrap_aligned;         
  wire                      wrap_is_incr;         
  wire                      wrap_split;           
  wire                      wrap_fits;            
  wire [11:0]                wrap_fits_mask;       
  wire [3:0]                working_addr;         

  wire                      wrap_split_reg_nxt;
  wire                      wrap_split_reg_wr_en;
  wire                      wrap_info_write_en;
  wire [11:0]                next_wrap_bytes_remaining_reg;
  wire [11:0]                addr_less_one;
  ...
  wire [19:0]                bytes_to_transfer_large;
  wire [11:0]                bytes_to_transfer_wrap1;
  ...
  wire [11:0]                bytes_in_transfer;
  wire [11:0]                bytes_in_transfer_aligned;
 ...
 assign bdata_valid = new_transaction;
  ...
  assign trans_split_resp_incr = bytes_to_transfer_large[10:7] + 4'b1;
  assign trans_split_resp = (burst_overflow) ? trans_split_resp_incr :
                            (no_long_bursts) ? bytes_to_transfer_large[10:7]    : 
                                               bytes_to_transfer_large[14:11];

  assign wrap_split_overflow = |next_wrap_bytes_remaining_reg[11:7];
 ...
 assign addr_less_one = awaddr_s[11:0] - {{11{1'b0}},1'b1};
 ...
 always @(posedge aclk or negedge aresetn)
   begin : wrap_bytes_remaining_reg_p
      if (!aresetn) begin
          wrap_bytes_remaining_reg <= 12'b0;
      end else if (wrap_info_write_en) begin
          wrap_bytes_remaining_reg <= next_wrap_bytes_remaining_reg;
      end
    end

 assign awaddr_aligned = {awaddr_s[39:12],((awaddr_s[11:0]) & (~total_bytes))};
 ...
 always @(posedge aclk or negedge aresetn)
    begin : sticky_valid_reg
      if (!aresetn) begin
          bypass_merge_int       <= 1'b0;
          long_burst_int         <= 1'b0;
      end else if(!awvalid_s | (awvalid_s & awready_s)) begin
          bypass_merge_int       <= bypass_merge;
          long_burst_int         <= long_burst;
      end    
    end 
 ...
 assign bypass = (awburst_s == `AXI_ABURST_FIXED && (align_mask)) || ( (align_mask) ? ((~awcache_s[1]) || (awlen_s == `AXI_ALEN_1) || (bypass_merge_int) ) : 1'b0 );
 
 assign no_long_bursts = ~long_burst_int & ~(|awlen_s[7:4]);
 

 always @(*)
  begin : total_bytes_p
    case (awsize_s)
       `AXI_ASIZE_8    : total_bytes = {4'b0, awlen_s};
       `AXI_ASIZE_16   : total_bytes = {3'b0, awlen_s, 1'b1};
       `AXI_ASIZE_32   : total_bytes = {2'b0, awlen_s, 2'b11};
       `AXI_ASIZE_64   : total_bytes = {1'b0, awlen_s, 3'b111};
       `AXI_ASIZE_128  : total_bytes = {awlen_s, 4'b1111};
       `AXI_ASIZE_256,
       `AXI_ASIZE_512,
       `AXI_ASIZE_1024 : total_bytes = {4'b0, awlen_s};    
       default         : total_bytes = {12'bx};
    endcase
  end


 assign total_bytes_ext = total_bytes[6:0];
 ...
 assign wrap_fits_mask = {{9{1'b1}}, 3'b000};
 

 assign  wrap_fits = ~|(total_bytes & wrap_fits_mask) & (awburst_s == `AXI_ABURST_WRAP);
 assign  wrap_is_incr = ~|(awaddr_s[11:0] & total_bytes);
 ...
 assign bytes_to_transfer_wrap1 = ~(awaddr_s[11:0]) & wrap_boundary_mask;
 ...
 always @(*)
  begin : total_bytes_masked_p
    case (awsize_s)
       `AXI_ASIZE_8    : total_bytes_masked = {4'b0, awlen_s};
       `AXI_ASIZE_16   : total_bytes_masked = {3'b0, awlen_s, ~awaddr_s[0]};
       `AXI_ASIZE_32   : total_bytes_masked = {2'b0, awlen_s, ~awaddr_s[1:0]};
       `AXI_ASIZE_64   : total_bytes_masked = {1'b0, awlen_s, ~awaddr_s[2:0]};
       `AXI_ASIZE_128  : total_bytes_masked = {awlen_s, ~awaddr_s[3:0]};
       `AXI_ASIZE_256,
       `AXI_ASIZE_512,
       `AXI_ASIZE_1024 : total_bytes_masked = {4'b0, awlen_s};    
       default         : total_bytes_masked = {12{1'bx}};
    endcase
 end
  ...
  begin : align_mask_p
    case (awsize_s)
       ...
       `AXI_ASIZE_64   : align_mask = 1'b1;
       `AXI_ASIZE_128  : align_mask = 1'b0;
       `AXI_ASIZE_256,
       `AXI_ASIZE_512,
  ...
  always @(*)
   begin : total_bytes_fixed_p
      case (awsize_s)
        `AXI_ASIZE_8    : total_bytes_fixed = {12{1'b0}};
        `AXI_ASIZE_16   : total_bytes_fixed = {{11{1'b0}}, ~awaddr_s[0]};
        `AXI_ASIZE_32   : total_bytes_fixed = {{10{1'b0}}, ~awaddr_s[1:0]};
        `AXI_ASIZE_64   : total_bytes_fixed = {{9{1'b0}}, ~awaddr_s[2:0]};
        `AXI_ASIZE_128  : total_bytes_fixed = {{8{1'b0}}, ~awaddr_s[3:0]};
        `AXI_ASIZE_256,
       `AXI_ASIZE_512,
       `AXI_ASIZE_1024 : total_bytes_fixed = {12{1'b0}};    
        default         : total_bytes_fixed = {12{1'bx}};
    endcase
  end
  

 assign offset_address = (wrap_split_reg) ? awaddr_aligned[2:0] : 
                            ((trans_overflow_reg) ? awaddr_incr[2:0] : 
                            awaddr_s[2:0]);
 ...
 assign  bytes_lt_two_byte   = ~|bytes_to_transfer[11:1];
 assign  bytes_lt_two_half   = ~|bytes_to_transfer[11:1] & ~half_carry;
 assign  bytes_lt_four_half  = ~|bytes_to_transfer[11:2] & ~half_carry_double;
 assign  bytes_lt_four_word  = ~|bytes_to_transfer[11:2] & ~word_carry[2];
 assign  bytes_lt_eight_word = ~|bytes_to_transfer[11:3] & ~word_carry_double;
 ...
 always @(*)
  ...
  assign overflow = (trans_overflow & ~trans_overflow_reg) ? 1'b0 : final_address[3];
  ...
  assign incrlen = (overflow) ? incrlenmaxsize :  
                   (trans_overflow ?  {{4{~no_long_bursts}}, 4'b1111} : bytes_to_transfer_large[10:3]);

  
 assign downsize_m = downsize[3:3];
 ...
 assign burst_overflow = ~bypass & final_address[3] & ~trans_overflow_reg &
                                 (&({(bytes_to_transfer[10:7] | {4{no_long_bursts}}), bytes_to_transfer[6:3]}));
 

 assign trans_overflow = burst_overflow | ((no_long_bursts) ? (|bytes_to_transfer[11:7]) : bytes_to_transfer[11]);
 ...
 always @(posedge aclk or negedge aresetn)
  ...
  always @(posedge aclk or negedge aresetn)
   begin : incr_bytes_remaining_reg_p
      if (!aresetn) begin
          incr_bytes_remaining_reg <= 12'b0;
      end else if (incr_info_write_en) begin
          incr_bytes_remaining_reg <= next_incr_bytes_remaining_reg;
      end
    end

  assign base_address = (wrap_split_reg) ? awaddr_s[11:0] &  ~wrap_boundary_mask : awaddr_s[11:0];
  ...
  assign addr_reg_inc = ((no_long_bursts) ? 8'b00001000 : 8'b10000000); 
  ...
  assign addr_result = {inc_addr,base_address[3:0]} & 12'b111111111000;
  

  
  assign awaddr_incr = {awaddr_s[39:12],addr_result};


  assign bytes_in_transfer = {{1{1'b0}}, {4{~no_long_bursts}}, 4'b1111, {3{1'b1}}};     
                                          
  assign bytes_in_transfer_aligned = (trans_overflow & ~trans_overflow_reg) ? 
                  bytes_in_transfer & {{9{1'b1}}, ~awaddr_s[2:0]} : 
                  bytes_in_transfer;

                                          

  assign next_incr_bytes_remaining_reg = bytes_to_transfer - bytes_in_transfer_aligned - {{11{1'b0}},1'b1};
 ...
 always @(posedge aclk or negedge aresetn)
  ...
  assign wrap_overflow = (awburst_s == `AXI_ABURST_WRAP) ?
                          awvalid_s & (|total_bytes[11:7]) : 1'b0;


  
  assign boundary_incr = awaddr_aligned[11:7] + 5'b00001;

  assign awaddr_boundary = ({awaddr_aligned[39:12],boundary_incr,awaddr_aligned[6:0]});
  ...
  assert_never #(0,0,"ERROR, Transaction incoming that has awsize too large for incoming bus")
      ovl_max_input_size
       (
        .clk       (aclk),
        .reset_n   (aresetn),
        .test_expr (awvalid_s && (awsize_s > 4))
       );
