...
module nic400_switch2_maskcntl_ml6_sse710_main
  (
    awvalid_m,
    awready_m,
    arvalid_m,
  ...
  wire  [2:0]       next_rd_cnt;     
  wire  [2:0]       next_wr_cnt;     
  reg   [1:0]       next_mask_w;     
  reg   [1:0]       next_mask_r;     
  ...
  assign next_wr_cnt_empty = (next_wr_cnt == 3'd0);
  ...
  always @(next_wr_cnt or next_rd_cnt or next_full) 
   begin : p_mask_comb
     next_mask_w = {2{1'b0}};
     next_mask_r = {2{1'b0}};
     if (next_wr_cnt == 3'b100 || next_full)
        next_mask_w = {2{1'b1}};
     if (next_rd_cnt ==  3'b100)
        next_mask_r = {2{1'b1}};
  end 
