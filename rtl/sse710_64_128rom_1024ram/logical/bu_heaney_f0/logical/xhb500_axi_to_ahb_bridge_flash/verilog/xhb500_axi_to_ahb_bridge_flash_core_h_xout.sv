...
module xhb500_axi_to_ahb_bridge_flash_core_h_xout
(
  ...
  output      xhb500_axi_to_ahb_bridge_flash_pkg::ahb_mpayld_t                    hmpayld,

  input  wire logic                                                   hready,
  input  wire xhb500_axi_to_ahb_bridge_flash_pkg::ahb_spayld_t                    hspayld,


  output      logic                                                   rvalid,
  input  wire logic                                                   rready,
  output      xhb500_axi_to_ahb_bridge_flash_pkg::axi_rpayld_t                    rpayld,

  output      logic                                                   bvalid,
  input  wire logic                                                   bready,
  output      xhb500_axi_to_ahb_bridge_flash_pkg::axi_bpayld_t                    bpayld
);

  wire xhb500_axi_to_ahb_bridge_flash_pkg::ahb_spayld_t                           hs = hspayld;
  ...
  xhb500_axi_to_ahb_bridge_flash_pkg::xhb_ahb_addrph_t    ahb_addrph_q;
  ...
  xhb500_axi_to_ahb_bridge_flash_pkg::xhb_ahb_addrph_t    ahb_addrph;

  assign ahb_addrph.haddr_strb    = ahb_in.hfix.haddr_11_0[$clog2(16)-1:0];
  ...
  logic      [128-1:0]                    acc_hrdata_q;
  logic                                   acc_hresp_q;
  logic                                   acc_hresp_wr_q;


  wire logic [16-1:0] acc_wstrb_mask;
  xhb500_axi_to_ahb_bridge_flash_strbgen u_strb_mask
  (
  ...
  wire logic [128-1:0] acc_mask;
  genvar i;
  for (i = 0; i < 16; i=i+1) begin: g_mask
  ...
  wire logic [128-1:0] acc_hrdata_en = {128{acc_trans_valid & ~ahb_addrph_q.hwrite & ~ahb_addrph_q.ctrl.rvalid}} & acc_mask;

  genvar i_out;
  for (i_out = 0; i_out < 128; i_out=i_out+1) begin: g_hrdata
    if(~|((16-(i_out/8)) & ((16-1)-(i_out/8)))) begin: g_hrdata_unused
      always_comb begin
  ...
  wire logic [128-1:0] acc_hrdata = (acc_hrdata_q & {128{ahb_addrph_q.ctrl.resp_acc}}) | (hs.hrdata & acc_mask);
  ...
  wire xhb500_axi_to_ahb_bridge_flash_pkg::axi_rpayld_t  rreg_rpayld_in;
  ...
  xhb500_axi_to_ahb_bridge_flash_respreg_r
  #(
  .PAYLD_WIDTH          ($bits(xhb500_axi_to_ahb_bridge_flash_pkg::axi_rpayld_t))
  ) u_respreg_r
  (
    .clk                (clk),
    .resetn             (resetn),
  ...
  wire xhb500_axi_to_ahb_bridge_flash_pkg::axi_bpayld_t    rreg_bpayld_in;

  assign rreg_bpayld_in.bid   = ahb_addrph_q.axid;
  assign rreg_bpayld_in.bresp = acc_hresp_wr;

  xhb500_axi_to_ahb_bridge_flash_respreg_b
  #(
  .PAYLD_WIDTH          ($bits(xhb500_axi_to_ahb_bridge_flash_pkg::axi_bpayld_t))
  ) u_respreg_b
  (
    .clk                (clk),
    .resetn             (resetn),
