...
module nic400_switch1_wr_spi_tt_s0_sse710_sctrl_apb
  (
    aw_enable,
    tt_enable,
    wr_enable,
    ...
    output [2:0] aw_enable;     
    output [2:0] tt_enable;     
    output [2:0] wr_enable;     
    input  [2:0] asel;     
    input        aready;     
    input        wvalid;     
    input        wready;     
  ...
  reg   [2:0]    reg_tt_en;     
   ...
   always @(resp_valid_vector or ptr0 or ptr1 or ptr2)
   begin : p_lrg_comb
        int_tt_en = {3{1'b0}};
        ptr_sel = {2{1'b0}};
        shift = {3{1'b0}};
   ...
   assign next_ptr2 = shift[2] ? ptr_sel : ptr2;


    assign next_tt_reg = (add_push) ? asel 
                        : (tt_cnt == 2'b01 && dec_tt_cnt) ? {3{1'b0}}
                        : tt_reg[tt_sel];
   ...
   assign aw_enable = {3{aw_enable_bit}} & asel;

   assign wr_enable = tt_reg[tt_sel] & {3{valid_add}};
   ...
   assert_implication #(0,0,"ERROR, LRG list item 2 incorrectly updated")
     ovl_lrg_list_update2
      (
       ...
       .consequent_expr ((ptr2 == ptr_sel_reg)) 
     );
