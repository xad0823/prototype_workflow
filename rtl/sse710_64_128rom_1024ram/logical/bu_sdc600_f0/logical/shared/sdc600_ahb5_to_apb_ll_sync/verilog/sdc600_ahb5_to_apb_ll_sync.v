...
module sdc600_ahb5_to_apb_ll_sync #(
 ...
 (
  ...
  begin : direct_control
    ...
    sdc600_mux #(.DATA_WIDTH(ADDR_WIDTH-2) ) u_paddr_mux   ( .in_a(addr_reg),          .in_b(haddr[ADDR_WIDTH-1:2]),          .sel(direct_tr), .out_y(paddr[ADDR_WIDTH-1:2]) );
    sdc600_mux                               u_pwrite_mux  ( .in_a(wr_reg),            .in_b(hwrite),                         .sel(direct_tr), .out_y(pwrite) );

    assign psel    = (state_reg==ST_TRNF) | (state_reg==ST_TRNF2) | direct_tr;
    assign penable = (state_reg==ST_TRNF2);
    sdc600_mux #(.DATA_WIDTH(           3) ) u_pprot_mux   ( .in_a(pprot_reg),         .in_b(pprot_nxt),                      .sel(direct_tr), .out_y(pprot) );
    sdc600_mux #(.DATA_WIDTH(           4) ) u_pstrb_mux   ( .in_a(pstrb_reg[3:0]),    .in_b(pstrb_nxt),                      .sel(direct_tr), .out_y(pstrb) );
    sdc600_mux #(.DATA_WIDTH(MASTER_WIDTH) ) u_pmaster_mux ( .in_a(pmaster_reg),       .in_b(hmaster),                        .sel(direct_tr), .out_y(pmaster) );
  end
  else
