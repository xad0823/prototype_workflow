...
//      Version Information
//
//      Checked In          : Fri Sep 2 14:49:55 2016 +0100
//
//      Revision            : 277130a
//
...
wire max_burst = burst_count == MAX_ALLOCATED-1;

wire rearbitration_point = ~hlock_arb && ~waiting && (
                                          prev_no_port ||
                                          ~lastport_hselm ||
                                          (lastport_htransm == TRN_IDLE ) ||
                                          (lastport_htransm == TRN_NONSEQ && ~(lastport_hburstm == BUR_INCR && ~max_burst)) ||
                                          (lastport_htransm == TRN_SEQ && (lastport_hburstm == BUR_INCR && max_burst)) );

  always @ (negedge hresetn or posedge hclk)
    begin : p_burst_count
      if (~hresetn)
        burst_count          <= {BURST_COUNT_WIDTH{1'd0}};
      else
        if (hreadym)
          if (selected_port != prev_port || no_port )
            burst_count         <= {BURST_COUNT_WIDTH{1'd0}};
          else if (~no_port && ~max_burst)
...
//------------------------------ << start rridx >> -----------------------------
              <<rrelse>>if (req_port_<<rridx_name>>)
                selected_port = <<idw_si_v>>'b<<bin_rridx>>;
//------------------------------ << end rridx >> -------------------------------
              else if (lastport_hselm)
                selected_port = <<idw_si_v>>'b<<bin_rrin>>;
              else
                no_port = 1'b1;
            end
