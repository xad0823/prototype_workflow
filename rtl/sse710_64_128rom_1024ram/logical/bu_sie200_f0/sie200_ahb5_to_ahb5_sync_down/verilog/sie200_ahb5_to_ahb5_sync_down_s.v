...
module sie200_ahb5_to_ahb5_sync_down_s #(
 ...
 (
  ...
  output wire                     hsels_i,
  output wire [ADDR_WIDTH-1:0]    haddrs_i,
  output wire [1:0]               htranss_i,
  output wire [DATA_WIDTH-1:0]    hwdatas_i,
  output wire [USER_WIDTH-1:0]    hwusers_i,
...
wire                     hsels_i_ngo;
wire [ADDR_WIDTH-1:0]    haddrs_i_ngo;
wire [1:0]               htranss_i_ngo;
wire [DATA_WIDTH-1:0]    hwdatas_i_ngo;
wire [USER_WIDTH-1:0]    hwusers_i_ngo;
wire                     transs_req_ngo;
wire                     unlocks_req_ngo;
wire                     transs_hold_ngo;
...
sie200_ahb5_to_ahb5_sync_down_core_s # (
    .AW                 (ADDR_WIDTH   ),
    .DW                 (DATA_WIDTH   ),
    .MW                 (MASTER_WIDTH ),
    .UW                 (USER_WIDTH   ),
  ...
  u_core_s(
    ...
    .haddr_s         (haddrs_i_ngo      ),
    .htrans_s        (htranss_i_ngo     ),
    .hsize_s         (hsizes_i          ),
    .hwrite_s        (hwrites_i         ),
    .hready_s        (hreadys_i         ),
    .hprot_s         (hprots_i          ),
    ...
    .transs_hold     (transs_hold_ngo   ),
    .bwerr           (ahb5_sync_down_irq),
  ...
  if (WRITE_BUFFER == 1) begin: gen_wb_support
    ...
    sie200_ahb5_write_buffer # (
      ...
      u_wb(
        ...
        .haddr_m          (haddrs_i_ngo ),
        .htrans_m         (htranss_i_ngo),
        .hsize_m          (hsizes_i     ),
        .hwrite_m         (hwrites_i    ),
        .hready_m         (hreadys_i    ),
        .hprot_m          (hprots_i     ),
        ...
        .hwdata_m         (hwdatas_i_ngo),
        .hburst_m         (hbursts_i    ),
        .hexcl_m          (hexcls_i     ),
        .hauser_m         (hausers_i    ),
        .hwuser_m         (hwusers_i_ngo),
        ...
        .hruser_m         (hrusers_i    ),
        .wb_disable       (wb_disable   ),
        .bwerr_clear      (ahb5_sync_down_irq_clear),
        .bwerr            (ahb5_sync_down_irq      )
    );

  end
  else begin: gen_no_wb_support
    assign hsels_i_ngo   = hsel_s;
    assign hnonsecs_i    = hnonsec_s;
    assign haddrs_i_ngo  = haddr_s;
    assign htranss_i_ngo = htrans_s;
    ...
    assign ahb5_sync_down_irq = 1'b0;
  end
endgenerate
...
generate
  if (QS_POWER_EN == 1) begin: gen_force_iso
    ...
    sie200_and #(.DATA_WIDTH(1           )) u_and_hsels_i          (.in_a(~brg_pwr_req_s                 ), .in_b(hsels_i_ngo       ), .out_y(hsels_i       ));
    sie200_and #(.DATA_WIDTH(ADDR_WIDTH  )) u_and_haddrs_i         (.in_a({ADDR_WIDTH{~brg_pwr_req_s}}   ), .in_b(haddrs_i_ngo      ), .out_y(haddrs_i      ));
    sie200_and #(.DATA_WIDTH(2           )) u_and_htranss_i        (.in_a({2{~brg_pwr_req_s}}            ), .in_b(htranss_i_ngo     ), .out_y(htranss_i     ));
    sie200_and #(.DATA_WIDTH(DATA_WIDTH  )) u_and_hwdatas_i        (.in_a({DATA_WIDTH{~brg_pwr_req_s}}   ), .in_b(hwdatas_i_ngo     ), .out_y(hwdatas_i     ));
    sie200_and #(.DATA_WIDTH(USER_WIDTH  )) u_and_hwusers_i        (.in_a({USER_WIDTH{~brg_pwr_req_s}}   ), .in_b(hwusers_i_ngo     ), .out_y(hwusers_i     ));
    ...
    sie200_or  #(.DATA_WIDTH(1           )) u_and_transs_hold      (.in_a(brg_pwr_req_s                  ), .in_b(transs_hold_ngo   ), .out_y(transs_hold   ));
  end
    ...
    assign hsels_i        =  hsels_i_ngo;
    assign haddrs_i       =  haddrs_i_ngo;
    assign htranss_i      =  htranss_i_ngo;
    assign hwdatas_i      =  hwdatas_i_ngo;
    assign hwusers_i      =  hwusers_i_ngo;

    assign transs_req     =  transs_req_ngo;
    assign unlocks_req    =  unlocks_req_ngo;
    assign transs_hold    =  transs_hold_ngo;
  end
endgenerate
