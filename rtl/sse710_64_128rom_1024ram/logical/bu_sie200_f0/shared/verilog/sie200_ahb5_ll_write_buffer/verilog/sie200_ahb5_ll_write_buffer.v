...
module sie200_ahb5_ll_write_buffer
  #(
   parameter       AW    = 32,
   parameter       DW    = 32,
   parameter       MW    = 4,
  ...
  begin : p_reg_locked_trans
    ...
    else if (hclk_en & trans_valid_rdy & hmastlock_s) begin
      reg_locked_trans <= 1'b1;
    end
    else if (hclk_en & hready_s & ((hsel_s & (~hmastlock_s)) | (~hsel_s))) begin
      reg_locked_trans <= 1'b0;
    end
  end
  ...
  begin : p_reg_unlock_pend
    ...
    else if (hclk_en & clr_unlock_pend) begin
      reg_unlock_pend <= 1'b0;
    end
    else if (hclk_en & set_unlock_pend) begin
  ...
  always @(posedge hclk or negedge hresetn)
    ...
    if (!hresetn)
      reg_buf_pend <= 1'b0;
    else if (hclk_en & (set_buf_pend | clr_buf_pend))
  ...
  begin : p_reg_hwdata
    ...
    else if (hclk_en & reg_bufferable_m) begin
      reg_hwdata <= hwdata_s;
      reg_hwuser <= hwuser_s;
    end
  end
  ...
  begin : p_reg_bwerr
    ...
    else if (hclk_en & bwerr_clear) begin
       reg_bwerr <= 1'b0;
    end
    else if (hclk_en & reg_bwerr_set) begin
      reg_bwerr <= 1'b1;
    end
  end
