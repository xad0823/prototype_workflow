...
#      Version Information
#
#      Checked In          : Tue May 16 14:29:49 2017 +0100
#
#      Revision            : cfcb2ba
#
...
#                            -endian=<endianness>        byte-big, word-big (big) or little (default)
...
#                            -arch=v6|ahb2|ahb5          default = ahb5
# ------------------------------------------------------------------------------
...
if ($verbose == 1)   {
  print "\n=========================================================================\n".
        ...
        "= only be used by a person authorised under and to the extent permitted\n".
        "= by a subsisting licensing agreement from ARM Limited or its affiliates.\n".
        "=       (C) COPYRIGHT 2002-2012,2015-2016 ARM Limited or its affiliates.\n".
        "=           ALL RIGHTS RESERVED\n".
        "= This entire notice must be reproduced on all copies of this file\n".
        "= and copies of this file may only be made by a person if such person is\n".
        "= permitted to do so under the terms of a subsisting license agreement\n".
        "= from ARM Limited or its affiliates.\n".
        "=\n".
        "= Script version : SIE-200 (ADK3 syntax)\n";
...
if ($help == 1) {
  print "Purpose:\n".
        ...
        "   -adk1|adk2              read an ADK1v1|ADK2 input stimulus\n".
        "   -busWidth=<32|64>       default = 64\n".
        "   -endian=<little|(word-)big|byte-big>  default = little\n".
        "                           (word-big is BE-32 'ARM big-endian',\n".
        "                           (byte-big is BE-8)\n".
...
$arch = lc($arch);
if ($arch ne 'ahb5' and $arch ne 'ahb2' and $arch ne 'v6') {
  $arch = 'ahb5';
  warn "******************* WARNING 133 *******************\n".
       "The architecture selected is invalid\n".
       "Architecture must be ahb5, ahb2 or V6.\n".
       "ahb5 will be used by default.\n\n";
}
...
# Check for options incompatible with ADK2 stimulus
if ($adk2 == 1) {
  # Check architecture is AHB2 or V6
  if ($arch eq 'ahb5') {
    $arch = 'ahb2';
    warn "******************* WARNING 137 *******************\n".
         "ADK2v0 stimulus format has been selected.\n".
         "ahb2 architecture will be used by default.\n\n";
  }
}
...
$endian = lc($endian);
$endian = 'word-'.$endian if ($endian eq 'big');
if ($endian ne 'word-big' and $endian ne 'byte-big' and $endian ne 'little') {
  $endian = 'little';
  warn "******************* WARNING 138 *******************\n".
       "The endianness selected is invalid\n".
       "Endianness must be little, byte-big or (word-)big.\n".
       "little will be used by default.\n\n";
}
...
if ($verbose == 1) {
  ...
  print "ADK2 stimulus format selected\n"
    if ($adk2 == 1);
  print "ADK3 stimulus format selected\n"
    if ($adk1 == 0 and $adk2 == 0);
...
while (<INPUTFILE>) {
  ...
  if (/\S/) {
    ...
    # Check for adk2 option switch in the stimulus file
    } elsif (s/^\s*-adk2\b//i) {

      $adk2 = 1;

      if ($verbose == 1) {
        printf "ADK2 stimulus format selected ".
               "in stimulus file\n", $lineNum;
      }
      ...
      if ($arch eq 'ahb5') {
        $arch = 'ahb2';
        warn "******************* WARNING 137 *******************\n".
             "ADK2 stimulus format has been selected.\n".
             "ahb2 architecture will be used by default.\n\n";
      }
      ...
      if ($currCmd eq 'S' or $currCmd eq 'B') {
        ...
        if (GetTransCount() == 0 && (GetIncrCount() == 0)) {
            ...
            printf "Line %4d: ERROR 84: $currCmd command outside burst".
                   " (check burst size)\n", $lineNum;
            $fileErrors++;
        } elsif ($sequence{'EXCL'} == 1) {
            printf "Line %4d: ERROR 91: $currCmd command after exclusive $prevCmd command\n", $lineNum;
            $fileErrors++;
        }
...
sub CheckArraySize {
  ...
  if (($verbose == 1) and ($memorySize > $stimarraysize)) {
    ...
    print  "             STIMULUS_ARRAY_SIZE generic/parameter and use script\n";
...
sub OutputHex  {
  ...
  if    ($command{'CMD'} eq '0000') {$~ = 'WRITECMD'}
  elsif ($command{'CMD'} eq '0001') {$~ = 'READCMD'}
  elsif ($command{'CMD'} eq '0010') {$~ = 'SEQCMD'}
  elsif ($command{'CMD'} eq '0011') {$~ = 'BUSYCMD'}
  elsif ($command{'CMD'} eq '0100') {$~ = 'IDLECMD'}
  elsif ($command{'CMD'} eq '0101') {$~ = 'POLLCMD'}
  elsif ($command{'CMD'} eq '0110') {$~ = 'LOOPCMD'}
  elsif ($command{'CMD'} eq '0111') {$~ = 'COMMCMD'}
  elsif ($command{'CMD'} eq '1000') {$~ = 'QUITCMD'}
  else {
...
sub ParseW {
  ...
  $command{'NONSEC'}  = GetNonsecField();
  ($command{'EXCL'}, $command{'EXOKAY'}) = GetExclField();
  $command{'RESP'}    = GetWriteRespField();
  $command{'UNALIGN'} = GetUnalignField();
  $command{'BSTRB'}   = GetBstrbField();
  $command{'AUSER'}   = GetAUserField();
  $command{'DUSER'}   = GetDUserField();
  ...
  # Check that Exclusive access in only set for SINGLE or INCR burst
  CheckExclBurst($command{'BUR'}, $command{'EXCL'});
...
sub ParseS {
  ...
  $command{'NONSEC'} = $sequence{'NONSEC'};
...
# ------------------------------------------------------------------------------
# Get address phase user signal field value
#
# I/O: searches from start of line for the address phase user signal field
#      returns the value of the address phase user signal field,
#      or reports an error
# ------------------------------------------------------------------------------
sub GetAUserField {

  my $auser;             # (hex string) value of address phase user signal field in stimulus
  #my $auserFieldWidth;   # width of data field, i.e. number of hex digits

  # Note 'X' is allowed, but may cause an error in the FileReadCore
  if (s/\bua(?:0x)?([0-9A-FX]{1,32})\b//i) {
    if ($arch ne 'ahb5') {
      printf "Line %4d: ERROR 97 AHB5 fields are not allowed".
             " unless AHB5 architecture is selected: User Signal\n", $lineNum;
      $fileErrors++;
      $auser = 0;
    }
    $auser = $1;
...
# ------------------------------------------------------------------------------
# Get data phase user signal field value
#
# I/O: searches from start of line for the data phase user signal field
#      returns the value of the data phase user signal field,
#      or reports an error
# ------------------------------------------------------------------------------
sub GetDUserField {

  my $duser;             # (hex string) value of data phase user signal field in stimulus
  #my $duserFieldWidth;   # width of data field, i.e. number of hex digits

  # Note 'X' is allowed, but may cause an error in the FileReadCore
  if (s/\bud(?:0x)?([0-9A-FX]{1,32})\b//i) {
    if ($arch ne 'ahb5') {
      printf "Line %4d: ERROR 97 AHB5 fields are not allowed".
             " unless AHB5 architecture is selected: User Signal\n", $lineNum;
      $fileErrors++;
    ...
    if ($adk2 == 0) {
        ...
        if (s/\bp([0-1]{7})\b//i) {
          ...
          if ($arch ne 'ahb5') {
            ...
            printf "Line %4d: ERROR 97: AHB5 fields are not allowed".
                   " unless AHB5 architecture is selected: Prot field\n", $lineNum;
            $fileErrors++;
...
# ------------------------------------------------------------------------------
# Get Security field value
#
# I/O: searches along the current line for a valid security field value
...
sub GetNonsecField {
  ...
  # Check that the correct architecture is used
  if  ( $arch ne 'ahb5' ) {
    printf "Line %4d: ERROR 97: AHB5 fields are not allowed".
           " unless AHB5 architecture is selected: Nonsec field\n", $lineNum;
    $fileErrors++;
    $nonsec = '1';
  }
...
# ------------------------------------------------------------------------------
# Get Exclusive field value
#
# I/O: searches along the current line for a valid exclusive access field value
#      returns the value of the exclusive access and exclusive okay fields
# ------------------------------------------------------------------------------
sub GetExclField {
  ...
  if  ( $arch ne 'ahb5' ) {
    printf "Line %4d: ERROR 97: AHB5 fields are not allowed".
           " unless AHB5 architecture is selected: Excl field\n", $lineNum;
    $fileErrors++;
    $excl = '0';
    $exokay = '0';
  }
...
my $resp = shift;
  ...
  if ($resp eq '11' and $prot !~ '[01]1[0-1]{5}') {
    printf "Line %4d: ERROR 104: XFAIL response can only be specified".
           " for a W command with Prot[5] asserted\n", $lineNum;
    $fileErrors++;
  }
...
# ------------------------------------------------------------------------------
# Checks the burst type when the AHB5 exclusive access is set
# ------------------------------------------------------------------------------
sub CheckExclBurst {
  ...
  if ($excl == 1) {
    if ($burst ne '000' and $burst ne '001') {  # not (single or incr)
      printf "Line %4d: ERROR 90: Burst type must be".
             " SINGLE/INCR for exclusive transfers.\n", $lineNum;
      $fileErrors++;
    }
  }
...
sub AlignDataMask {
  ...
  $offset = MakeOffsetBigEndian( $offset ) if ($endian eq 'word-big');
...
sub CheckUnalignBstrb {
  ...
  $offsetAligned = MakeOffsetBigEndian( $offsetAligned ) if ($endian eq 'word-big');
  ...
  if ($endian eq 'byte-big' or $endian eq 'word-big') {
  ...
  # Not allowed to have a sparse/unaligned transfer in big-endian modes
  if (($endian eq 'word-big') and not $idleTransfer and $unalign eq '1') {
    printf "Line %4d: ERROR 103: Transfer cannot be unaligned or sparse".
           " in word-invariant big-endian mode\n", $lineNum;
    $fileErrors++;
  }
