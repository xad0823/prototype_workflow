...
module ca53_ramtest_number2     #(parameter RAM_ID      = 4'b0000,  // Unique ID
  ...
  localparam [RAM_ST_W-1:0] RAM_ST_Z_WRITE  = 3'b011;
  localparam [RAM_ST_W-1:0] RAM_ST_Z_READ   = 3'b100;
  localparam [RAM_ST_W-1:0] RAM_ST_F_WRITE  = 3'b101;
  localparam [RAM_ST_W-1:0] RAM_ST_F_READ   = 3'b110;  
  localparam [RAM_ST_W-1:0] RAM_ST_DONE     = 3'b111;
 ...
 always @ (checking) begin
  ...
  // Bank Count
  //-----------
  assign bank_cnt_en  = ((state == RAM_ST_READ) & word_cnt == WORD_WIDTH) | state == RAM_ST_DONE | (state == RAM_ST_IDLE & word_cnt == {WORD_WIDTH{1'b0}});

  assign bank_cnt_nxt = ((state == RAM_ST_READ) & (bank_cnt < bank_cnt_end)) ? bank_cnt + 4'h1 : next_state == RAM_ST_Z_WRITE ? bank_cnt_end : 4'h0;
  ...
  assign word_cnt_nxt = ((state == RAM_ST_READ) & (word_cnt < WORD_WIDTH)) ? word_cnt + 8'h1 : 8'h0;
  ...
  always @*
    case (state)
      ...
      RAM_ST_READ     : next_state = (bank_cnt == bank_cnt_end ) & (word_cnt == WORD_WIDTH) ? RAM_ST_Z_WRITE : RAM_ST_WRITE;
      RAM_ST_Z_WRITE  : next_state = RAM_ST_Z_READ;
      RAM_ST_Z_READ   : next_state = RAM_ST_F_WRITE;
      RAM_ST_F_WRITE  : next_state = RAM_ST_F_READ;
      RAM_ST_F_READ   : next_state = RAM_ST_DONE;
 ...
 // Set the ADDR and WORD for each stage
  always @(posedge clk or negedge reset_n) begin
    if(~reset_n) begin
      ...
      addr_o      <= {ADDR_WIDTH{1'b0}};
      wdata_raw   <= { {(WORD_WIDTH-1){1'b0}},1'b1 } << word_cnt;
    end  else if (state == RAM_ST_Z_WRITE) begin
      addr_o      <= {ADDR_WIDTH{1'b0}};
      wdata_raw   <= {WORD_WIDTH{1'b0}};
    end  else if (state == RAM_ST_Z_READ) begin
      addr_o      <= {ADDR_WIDTH{1'b0}};
      wdata_raw   <= {WORD_WIDTH{1'b1}};
    end else if (state == RAM_ST_F_WRITE) begin 
      addr_o      <= {ADDR_WIDTH{1'b0}};
      wdata_raw   <= {WORD_WIDTH{1'b0}};
    end else if (state == RAM_ST_F_READ) begin 
      addr_o      <= {ADDR_WIDTH{1'b0}};
      wdata_raw   <= {WORD_WIDTH{1'b0}};          
...
// Set the Enable signals for each stage
  always @(posedge clk or negedge reset_n) begin
    if(~reset_n) begin
      ...
      wr_en_o     <= 1'b1;
      wrbyte_en_o <= {NUM_STRBS{1'b1}};
    end else if (next_state ==  RAM_ST_Z_WRITE & state == RAM_ST_READ) begin
      enable_o    <= {NUM_BANKS{1'b1}};
      wr_en_o     <= 1'b1;
      wrbyte_en_o <= {NUM_STRBS{1'b1}};
    end else if (next_state ==  RAM_ST_Z_READ & state == RAM_ST_Z_WRITE) begin
      enable_o    <= {NUM_BANKS{1'b1}};
      wr_en_o     <= 1'b0;
      wrbyte_en_o <= {NUM_STRBS{1'b0}};
    end else if (next_state ==  RAM_ST_F_WRITE & state == RAM_ST_Z_READ) begin
      enable_o    <= {NUM_BANKS{1'b0}};
      wr_en_o     <= 1'b1;
      wrbyte_en_o <= {NUM_STRBS{1'b1}};
    end else if (next_state ==  RAM_ST_F_READ & state == RAM_ST_F_WRITE) begin
      enable_o    <= {NUM_BANKS{1'b1}};
      wr_en_o     <= 1'b0;
      wrbyte_en_o <= {NUM_STRBS{1'b0}};   
    end  else if (next_state ==  RAM_ST_DONE & state == RAM_ST_F_READ) begin
      enable_o    <= {NUM_BANKS{1'b0}};
      wr_en_o     <= 1'b0;
      wrbyte_en_o <= {NUM_STRBS{1'b0}};
    end
 ...
 assign wdata_check_o  = wdata_raw;
 ...
 always @(negedge clk or negedge reset_n) begin
    if (!reset_n) begin
    ...
    end else if (word_error < 10 & (selected_state == RAM_ST_READ | selected_state == RAM_ST_Z_READ | selected_state == RAM_ST_F_READ)) begin
      ...
      if ((rdata_i !=  selected_data) || (rdata_i !== selected_data)) begin
	    ...
	    $display ("%9dns  Checking %0s  [Addr0x%X] Word Error (Walking ones in wdata).\n Expected = 0x%X, \n Actual   = 0x%X", $time, check_string,
                            selected_addr, selected_data, rdata_i);
	  end else begin
	    $display ("%9dns  Checking %0s  %0s %1d [Addr0x%X] Word Error (Walking ones in wdata).", $time, check_string, bank_string, selected_bank_cnt,
                            selected_addr);
	    ...
	    begin : CHUNK_SELECTION_GENERATE
          ...
          $display ("%9dns  Checking %0s  %0s %1d [Addr0x%X] Word Matched (Walking ones in wdata).\n Expected = 0x%X, \n Actual   = 0x%X", $time, check_string, bank_string, selected_bank_cnt,
                          selected_addr, selected_data, rdata_i);
      end
     ->checking;
    end
