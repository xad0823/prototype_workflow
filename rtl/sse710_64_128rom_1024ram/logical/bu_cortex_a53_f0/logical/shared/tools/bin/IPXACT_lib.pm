...
#       Checked In          : $Date: 2010-04-27 17:34:52 +0100 (Tue, 27 Apr 2010) $
#       Revision            : $Revision: 7386 $
#       Release             : CORTEXA53-r0p4-00rel0
#
#******************************************************************************
#
...
BEGIN {
  ...
  $VERSION = sprintf "%d", q$Revision: 7386 $ =~ /(\d+)/;
  @ISA = qw(Exporter);
  @EXPORT = qw(&check_XML &debugMsg &print_error &print_warning &solve_dependency &check_configuration &check_type &XPathSolve &check_file &remove_TPmarkup &get_filelist &getVLNV &get_child_element &getVLNVType);
  %EXPORT_TAGS = ( );
...
#sub print_error($) {}
#sub debugMsg($) {}
#sub get_child_element($$) {} ## shouldn't be needed really but...
...
sub remove_TPmarkup($) {
  ...
  $clean_string =~ s/\<!\[CDATA\[//g;  # remove of openning CDATA tags
  ...
  $clean_string =~ s/\<+?\>//g;     # remove of any tags (must be minimal to ensure it leaves bits between tags!)
                                    # this catches openning and closing tags
  return $clean_string;
...
sub solve_dependency ($) {
  ...
  if ($node_attributes{'spirit:resolve'} eq "dependent") {
    ...
    debugMsg("solve_dependency: not dependent spirit:dependency=".$node->nodeValue);
  ...
  if ($XPath_expr =~ /id\('(.*)'\)/) {
    ...
    while ($partial =~ /id\('(.*?)'\)/) {
      ...
      if ( $id_result->size() == 1) {
        ...
        my $alt_id = $id_result->get_node(1)->nodePath();
...
sub get_filelist ($) {
  ...
  if ($basedir =~ m#^http://# ) {
    ...
    if ( $@ ) {
      print_error("get_filelist: Error occured openning $basedir");
      # critical error
      $okay = 0;
    }
  ...
  SWITCH: {
      if ($param_attribute{'spirit:format'} =~ /bitString/) {
        ...
        if ($param_value =~ /([0-9]+)b([01]+)/s ) {
          ...
          # unknown
          print_error("check_configuration: unknown format type for bitString at node: ".$node->nodePath());
          $pass = 0;
        }
        last SWITCH;
      }
...
sub print_warning($) {
  ...
  if ($wiki) {
    # Display suitable for wiki pages
    print "%yellow% WARNING: ".$message."%ENDCOLOR% </br>";
  } elsif ($html) {
    # Display suitable for HTML pages
    print "<font color=\"yellow\">Warning: ".$message."</font></br>\n";
  } else {
    # No formatting
    print "$0 ERROR: $message\n";
  }
