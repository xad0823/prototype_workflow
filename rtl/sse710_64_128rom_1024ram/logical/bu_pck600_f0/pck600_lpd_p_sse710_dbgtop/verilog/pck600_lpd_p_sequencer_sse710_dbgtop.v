...
module pck600_lpd_p_sequencer_sse710_dbgtop#(
  parameter DEV_ID             = 0,
...
`include "pck600_lpd_p_shared_sse710_dbgtop.v"
    localparam WARM_RST_VAL  = 4'h9;
  localparam DBG_RECOV_VAL = 4'hA;

  wire new_reorder;
  wire prv_reorder;
  wire prv_pstate_res;

  assign new_reorder = (   (ctrl_pstate_i[3:0] == WARM_RST_VAL)
                         | (ctrl_pstate_i[3:0] == DBG_RECOV_VAL))
                       & (ctrl_pstate_i[3:0] != prv_ctrl_pstate_i[3:0]);

  assign prv_pstate_res = (   (prv_ctrl_pstate_i[3:0] == WARM_RST_VAL)
                            | (prv_ctrl_pstate_i[3:0] == DBG_RECOV_VAL));

  assign prv_reorder = prv_pstate_res & (ctrl_pstate_i[3:0] != prv_ctrl_pstate_i[3:0]);

      wire rtn_reorder;

    assign rtn_reorder = (   (new_ctrl_pstate_i[3:0] == WARM_RST_VAL)
                           | (new_ctrl_pstate_i[3:0] == DBG_RECOV_VAL)
                           | prv_pstate_res)
                         & (new_ctrl_pstate_i[3:0] != prv_ctrl_pstate_i[3:0]);
  
  always@*
  begin
    case(state)
`include "pck600_lpd_p_init_sse710_dbgtop.v"
    P_STABLE:
    begin
      state_en               = (ctrl_preq_i | request_accepted_r | return_flag_r);
      nxt_state              = (request_denied_i | ~ctrl_preq_i) ? P_STABLE
                             :   (ctrl_pstate_i[3:0] < prv_ctrl_pstate_i[3:0])
                               ? ((new_reorder | prv_reorder)
                                  ? (prv_dev_accepted_i
                                     ? (dev_p_ch_same_en
                                        ? P_REQUEST
                                        : ((mapped_ctrl_pstate != mapped_prv_pstate)
                                           ? P_REQUEST
                                           : P_ACCEPT))
                                     : P_STABLE)
                                  : (nxt_dev_accepted_i
    ...
    begin
      state_en               = 1'b1;
      nxt_state =   (new_ctrl_pstate_i[3:0] < prv_ctrl_pstate_i[3:0])
                  ? (rtn_reorder
                     ? (nxt_return_flag_i
                        ? (dev_p_ch_same_en
                           ? (dev_paccept_i
                              ? P_ACCEPT
                              : P_REQ_RTN)
                           : (mapped_new_pstate != mapped_prv_pstate)
                               ? (dev_paccept_i
                                  ? P_ACCEPT
                                  : P_REQ_RTN)
                               : P_ACCEPT)
      ...
      nxt_dev_preq_r =   (new_ctrl_pstate_i[3:0] < prv_ctrl_pstate_i[3:0])
                       ? (rtn_reorder
                          ? (nxt_return_flag_i
                             ? (dev_p_ch_same_en
                                ? ~(dev_paccept_i | dev_pdeny_i)
                                : ((mapped_new_pstate != mapped_prv_pstate)
                                   ? ~(dev_paccept_i | dev_pdeny_i)
                                   : 1'b0))
                             : 1'b0)
