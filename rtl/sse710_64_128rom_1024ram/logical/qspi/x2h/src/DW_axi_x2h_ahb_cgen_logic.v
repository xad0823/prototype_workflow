...
// Revision: $Id: //dwh/DW_ocb/DW_axi_x2h/axi_dev_br/src/DW_axi_x2h_ahb_cgen_logic.v#8 $ 
//
// -------------------------------------------------------------------------
...
// Inputs:
  ...
  input                        [1:0] to_process_ds_factor;
  ...
  //The signal LSbit of this signal is not used. Functionally it is not required.
       ...
       if (|(to_process_addr[4:0] & to_process_try_mask))
         ...
         begin
           ...
           if ((`X2H_AHB_DATA_WIDTH > 128) & (to_process_try_size == `X2H_AMBA_SIZE_32BYTE))
             ...
             //SJ : Data loss never occurs because of left shift as the most significant 5 bits are 0. 
...
function automatic [6:0] check_wstrb_function;
  ...
  begin
      ...
      shifted_wstrb = {{(32-`X2H_AXI_WSTRB_WIDTH){1'b0}}, wstrb} >> start_address[3:0];
...
function automatic [4:0] choose_bcount_function;
   ...
   begin
     ...
     //         and/or undefined-length INCRs
       ...
       begin
         if (to_process_ds_factor_int[1])
           begin   // 4:1 DOWNSHIFT
             burst16_ok  = (`X2H_READ_BUFFER_DEPTH > 4);
             burst8_ok   = (`X2H_READ_BUFFER_DEPTH > 2);
             burst4_ok   = 1'b1;
           end
         else if (to_process_ds_factor_int[0])
           begin   // 2:1 DOWNSHIFT
             burst16_ok  = (`X2H_READ_BUFFER_DEPTH > 8);
             burst8_ok   = (`X2H_READ_BUFFER_DEPTH > 4);
             burst4_ok   = (`X2H_READ_BUFFER_DEPTH > 1);
           end
         else
