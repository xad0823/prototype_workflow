...
// Revision: $Id: //dwh/DW_ocb/DW_axi_x2h/axi_dev_br/src/DW_axi_x2h_slave.v#7 $ 
//
// -------------------------------------------------------------------------
...
//  awready, 
  ...
  //When AXB_ADDR_WIDTH > AHB_ADDR_WIDTH, the extra MSBs of the awddr signal are unused.
...
//  `endif
  ...
  //When AXI_ADDR_WIDTH > AHB_ADDR_WIDTH, the extra MSBs of the arddr signal are unused.
...
//--------------------------------------------------------------------------
  ...
  //In case of AXI4 interface the MSB of the lock_c signals are tied to zero to make it compatible with AXI4 lock specification.
...
//-----------------------------------------------------------------------------     
  ...
  //LSB of the write cmd_q is always 1'b1.
...
//  assign awready = ~cmd_queue_rdy_n;
  ...
  always @(*) begin:SLAVE_CMD_Q_PROC
    ...
    if ( rd_push_comm_cmd_q_n != 1) begin
      cmd_queue_wd = rd_cmd_queue_wd;
    end else begin
      cmd_queue_wd = wr_cmd_queue_wd;
    end
  end  
