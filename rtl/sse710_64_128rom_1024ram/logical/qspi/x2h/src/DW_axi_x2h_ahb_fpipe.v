...
// Revision: $Id: //dwh/DW_ocb/DW_axi_x2h/axi_dev_br/src/DW_axi_x2h_ahb_fpipe.v#10 $ 
//
// -------------------------------------------------------------------------
...
// Interface to CGEN module... CMD Queue signals
   ...
   output                       [1:0] cmdq_ds_factor;      // Proposed downsize
                                                           // factor
...
// Interface to CGEN module... Write Data FIFO signals
  ...
  //Range 5-6. Bits 5,6 are not used when PASS_LOCK is disabled.
   reg    [`X2H_CMD_QUEUE_WIDTH-1:0] ff_hcmd_queue_wd_int;

   wire     [`X2H_CMD_ADDR_WIDTH-1:0] ff_addr;
  //When X2H is configured to single clock mode, the buffers are configured to pass through data without synchronization.
  //In this config the buffers can be configured to single entry deep. Waiving off the unused signals in this configuration.
   wire       [`X2H_AXI_ID_WIDTH-1:0] ff_id;
   wire                         [2:0] ff_axi_size;
   wire                         [1:0] ff_cache;
   wire                               ff_prot_2;
   wire                               ff_prot_0;
   wire                               ff_rw;
   wire                         [2:0] ff_try_ahb_size;
   wire                         [4:0] ff_try_ahb_mask;
  //When AXI_DATA_WIDTH : AHB_DATA_WIDTH is 2:1 (or 4:1, depending on the configuration), the MSBit of the to_process_ds_factor signal is unused.
   wire                         [1:0] ff_ds_factor;
   wire                        [11:0] ff_wrapmask;
   wire                        [13:0] ff_axibytes;
   wire                        [13:0] ff_axibytes_2b;
   wire                               ff_2b_first;
   wire            [`X2H_AXI_BLW-1:0] ff_frepcount;
   ...
   always @(*)
     ...
     begin:S_HCMD_PROC
           ...
           pop_hcmd_int_n = ~(pop_hcmd_pipeone & (~hcmd_rdy_int_n));
           hcmd_wd_pipeone = ff_hcmd_queue_wd_int;
   ...
   always @(*)
     begin:HCMD_POST_PROC
       ...
       // MS 4 bits are tied to zeros to retain the bit width.
   ...
   begin:FF_SIG_PROC
     if ( !rst_n )
       begin
         ff_hcmd_queue_wd_int      <= {`X2H_CMD_QUEUE_WIDTH{1'b0}};
         ff_cmdq_valid      <= 1'b0;
       ...
       // "ff_cmdq_valid" does not yet reflect it.
         ...
         if (pop_cmdq | hcmd_pipeone_rdy & (~ff_cmdq_valid))
           ...
           begin
             ff_hcmd_queue_wd_int <= hcmd_queue_wd_int;
       ...
       assign ff_addr           = raw_addr;
       assign ff_id             = ff_hcmd_queue_wd_int[`X2H_CMD_QUEUE_WIDTH-(`X2H_CMD_ADDR_WIDTH+1) : 12+`X2H_AXI_BLW];
       assign ff_axi_size       = raw_size;
       assign ff_cache          = ff_hcmd_queue_wd_int[4:3];
       assign ff_prot_2         = ff_hcmd_queue_wd_int[2];
       assign ff_prot_0         = ff_hcmd_queue_wd_int[1];
       assign ff_rw             = ff_hcmd_queue_wd_int[0];
       assign ff_try_ahb_size   = try_ahb_size;
       assign ff_try_ahb_mask   = try_ahb_mask;
       assign ff_ds_factor      = ds_factor;
       assign ff_wrapmask       = wrapmask;
       assign ff_axibytes       = blen_plus_1;
       assign ff_axibytes_2b    = blen_2b_plus_1;
       assign ff_2b_first       = blen_2b_is_less;
       assign ff_frepcount      = raw_frepcount;

//wrapmask function definition.
function automatic [11:0] wrapmask_function;
  ...
  begin
    if ( axi_burst_type != `X2H_AXI_BURST_WRAP )
      ...
      begin
        ...
        // SMD: Return type width is greater than return value width in function
        // SJ : This is a false error and it can be waived
