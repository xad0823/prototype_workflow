...
// Revision: $Id: //dwh/DW_ocb/DW_axi_x2h/axi_dev_br/src/DW_axi_x2h_ahb_cgen.v#11 $ 
//
// -------------------------------------------------------------------------
...
// Interface to Common CMD Queue (via the FPIPE)
   ...
   input                        [1:0] cmdq_ds_factor;      // Proposed downsize
                                                           // factor
...
// CGEN-IF interface (mostly via CPIPE)
  ...
  assign to_process_ds_factor  = cmdq_ds_factor;
 ...
 U_cgen_logic (
  ...
  // Inputs:
    ...
    .to_process_addr(to_process_addr[8:0]),
                                        ...
                                        .to_process_ds_factor(to_process_ds_factor),
...
//SJ : We are assigning cgen_cpipe_valid multiple times based on conditions as per the design requirement. So this warning can be ignored. 
       if (~wdfifo_valid)
         begin
...
//spyglass disable_block W415a
//SMD: Signal may be multiply assigned (beside initialization) in the same scope.
//SJ : We are assigning cgen_cpipe_rdfifo_req multiple times based on conditions as per the design requirement. So this warning can be ignored.   
   ...
   // be needed to perform it.
     ...
     begin
       if (to_process_ds_factor[1])
         // 4:1 downsize
         // 16-burst will take 4 places
         //  8-burst will take 2 places
         //  4-burst will take 1 place
         //  1-burst might take a place per xbytes_to_bump_axi
         cgen_cpipe_rdfifo_req = {2'b0, ahb_bcount[4:2]};
 
       else if (to_process_ds_factor[0])
         // 2:1 downsize
         // 16-burst will take 8 places
         //  8-burst will take 4 places
         //  4-burst will take 2 place
         //  1-burst might take a place per xbytes_to_bump_axi
         cgen_cpipe_rdfifo_req = {1'b0, ahb_bcount[4:1]};
       ...
       // Do this in any case
       // It would be more precise to qualify this with ahb_bcount[0],
       // but it turns out this won't matter to the IF section
...
//spyglass enable_block W415a
   ...
   //SJ: Underflow will never happen. Refer to description of ahb_xbytes.
...
// Depending on the got_write_err qualifier the hw_status_int signal takes a value that is a constant.
...
function automatic [`X2H_AHB_DATA_WIDTH-1:0] wdata_mux_function;
  input [4:0] address;
  input [`X2H_AXI_DATA_WIDTH-1:0] wdfifo_wdata;
