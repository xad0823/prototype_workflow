...
module nic400_ib_xnvm_axi_m_ib_upsize_rd_addr_fmt_sse710_integration_example_f0_flash
  (
    aresetn,
    aclk,
...
`include "Axi.v"
 ...
 assign ardata_data[`ARDATA_ADDR] = araddr_s[3:0] & align_mask;
 assign ardata_data[`ARDATA_MASK] = read_mask;
 assign ardata_data[`ARDATA_END] = ((arburst_s == `AXI_ABURST_WRAP) & ~wrap_is_incr) ? addr_less_one[3:0] & align_mask : final_address[3:0] & align_mask;
 ...
 assign araddr_aligned = {araddr_s[31:11],((araddr_s[10:0]) & (~total_bytes))};
 ...
 assign working_addr = araddr_s[4:0] & wrapalignmask;
 ...
 always @(*)
  begin : align_mask_p
    case (arsize_s)
       `AXI_ASIZE_8    : align_mask = {4{1'b1}};
       `AXI_ASIZE_16   : align_mask = {{3{1'b1}}, 1'b0};
       `AXI_ASIZE_32   : align_mask = {{2{1'b1}}, 2'b0};
       `AXI_ASIZE_64   : align_mask = {{1{1'b1}}, 3'b0};
       `AXI_ASIZE_128,
       `AXI_ASIZE_256,
       `AXI_ASIZE_512,
       `AXI_ASIZE_1024 : align_mask = {4{1'b1}};    
       default         : align_mask = {4{1'bx}};
    endcase
  end
  

 assign offset_address = (wrap_split_reg) ? araddr_aligned[3:0] : 
                            araddr_s[3:0];
...
`ifdef ARM_ASSERT_ON
  ...
  assert_never #(0,0,"ERROR, Transaction issued that is too large for outgoing bus")
      ovl_max_output_size
       (
        .clk       (aclk),
        .reset_n   (aresetn),
        .test_expr (arvalid_m && (arsize_m > 4))
       );
  ...
  assert_never #(0,0,"ERROR, Transaction should not have been touched")
      ovl_illegal_trans_mod
        ...
        .clk       (aclk),
        .reset_n   (aresetn),
        .test_expr (arvalid_m && (!arcache_s[1]) && (arsize_s <= 4)
