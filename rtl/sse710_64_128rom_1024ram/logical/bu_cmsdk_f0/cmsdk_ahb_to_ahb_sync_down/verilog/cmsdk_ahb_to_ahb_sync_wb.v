...
//
//            (C) COPYRIGHT 2011-2013,2017 Arm Limited or its affiliates.
//                ALL RIGHTS RESERVED
//
...
//           IDLE cycle will be inserted. (AMBA 3 recommendation)
 ...
 // --------------------------------------------------------------------------------
  ...
  reg              reg_locked_trans;  // Locked transfer in progress

  reg              reg_unlock_pend;   // A locked transfer is to be unlocked
  wire             set_unlock_pend;   // set reg_buf_pend
  wire             clr_unlock_pend;   // clear reg_buf_pend

  wire             idle_injection;    // Indicate idle injection is needed delaying HTRANS going to downstream AHB
  ...
  // Delayed version of hmastlock_s, when hsel_s is valid.
  always @(posedge HCLK or negedge HRESETn)
  begin : p_reg_locked_trans
    if (!HRESETn) begin
      reg_locked_trans <= 1'b0;
    end
    else if (HCLKEN & trans_valid_rdy & HMASTLOCKS) begin
      // locked transaction start with valid transaction
      reg_locked_trans <= 1'b1;
    end
    else if (HCLKEN & HREADYS & ((HSELS & (~HMASTLOCKS)) | (~HSELS))) begin
      // unlock the transaction if master unlocks it or current bridge is not selected any more
      reg_locked_trans <= 1'b0;
    end
  end
  ...
  // Slave side unlock condition to be stored as it cannot pass through the bridge immediately
  assign set_unlock_pend  = HREADYS & ~hreadym_mux & ~trans_valid & reg_locked_trans & (((~HMASTLOCKS) & HSELS) | (~HSELS)) & idle_inject_enable;
  ...
  begin : p_reg_unlock_pend
    ...
    else if (HCLKEN & clr_unlock_pend) begin
      //unlock idle injection done
      reg_unlock_pend <= 1'b0;
    end
    else if (HCLKEN & set_unlock_pend) begin
      //unlock idle injection requested
  ...
  assign idle_injection = (reg_bufwr_running & (~reg_buf_pend) & (~HREADYS)) | reg_unlock_pend;
  ...
  assign i_htrans    = (address_phase_output_sel  ? reg_htrans : HTRANSS) &
                       {(~idle_injection), 1'b1};
  ...
  assign i_hmastlock = reg_unlock_pend            ? 1'b0 :
