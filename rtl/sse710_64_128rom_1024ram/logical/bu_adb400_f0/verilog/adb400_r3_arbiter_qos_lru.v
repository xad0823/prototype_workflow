...
//-----------------------------------------------------------------------------
// File: adb400_r3_arbiter_qos_lru.v
//-----------------------------------------------------------------------------
// Purpose : QoS arbiter that resorts to LRU arbitration in the event of a
//           highest QoS value tie
//
//   The arbiter selects the input with the highest QoS value.  In the event of
//   a tie between highest QoS values it resorts to a Least Recently Used (LRU)
//   scheme.
//
// Usage :
...
module adb400_r3_arbiter_qos_lru
  //---------------------------------------------------------------------------
  ...
  (
    ...
    input   wire  [(WAYS*QV_SIZE)-1:0]  qos_i,
  ...
  generate
    ...
    // Create an array of QoS values from the concatenated input
    for (i=0; i<WAYS; i=i+1)
      begin: g_qos_array
        assign qos[i] = qos_i[(i*QV_SIZE)+:QV_SIZE];
      end // g_qos_array
    ...
    // * A has a greater QoS value than B
    // * A has a QoS value equal to that of B and is the LRU (least recently used)
      ...
      begin: g_sel_outer
        for (b=0; b<a; b=b+1)
          begin: g_sel_inner
            ...
            assign sel   [a][b] = ~request_i[b] | (request_i[a] & ((qos[a] > qos[b]) | ((qos[a] == qos[b]) & lru)));
