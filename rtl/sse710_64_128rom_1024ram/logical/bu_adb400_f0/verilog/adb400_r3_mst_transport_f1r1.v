...
module adb400_r3_mst_transport_f1r1
  #(parameter
      FW_FIFO_DEPTH       = 6,
      RV_FIFO_DEPTH       = 6,
      FW_OPREG            = 1,
  ...
  (
    ...
    output wire fwvalid,
    input  wire fwready,
    output wire [((FW_PAYLOAD_WIDTH>0)?(FW_PAYLOAD_WIDTH-1):0):0] fw_payld,

    // RV channel signalling
    input  wire rvvalid,
    output wire rvready,
    input  wire [((RV_PAYLOAD_WIDTH>0)?(RV_PAYLOAD_WIDTH-1):0):0] rv_payld,
    ...
    input  wire [((FW_PAYLOAD_WIDTH>0)?((FW_PAYLOAD_WIDTH*FW_FIFO_DEPTH)-1):0):0] fw_payld_async,

    // B channel signalling
    output wire [RV_FIFO_DEPTH-1:0] rv_wr_ptr_async,
    input  wire [RV_FIFO_DEPTH-1:0] rv_rd_ptr_async,
    output wire [((RV_PAYLOAD_WIDTH>0)?((RV_PAYLOAD_WIDTH*RV_FIFO_DEPTH)-1):0):0] rv_payld_async
  );
  ...
  wire   fwvalid_int;
  assign fwvalid      = (outer_fence_n & (responder_fence_n | fw_hndshk_sticky_q) & fwvalid_int);
  wire   fwready_int  = (outer_fence_n & (responder_fence_n | fw_hndshk_sticky_q) & fwready);

  reg    rv_hndshk_sticky_q;

  wire   rvvalid_int  = (outer_fence_n & (initiator_fence_n | rv_hndshk_sticky_q) & rvvalid);
  wire   rvready_int;
  assign rvready      = (outer_fence_n & (initiator_fence_n | rv_hndshk_sticky_q) & rvready_int);
  ...
  wire initiator_quiescentn_nxt    = (// Drive high because of a new transfer
                                      (rvvalid_int & rvready_int) |
                                      // Keep high till the transfer has completely left the channel
                                      (initiator_quiescentn_q & rv_chan_cactive)
                                     );
  ...
  wire initiator_quiescent_and_stalledn_nxt    = (// Drive high because of a new transfer
                                                  ...
                                                  initiator_fence_n |
                                                  // Keep high till the transfer has completely left the channel
                                                  (initiator_quiescent_and_stalledn_q & rv_chan_cactive)
                                                 );
  ...
  wire responder_quiescentn_nxt    = (// Drive high because of a new transfer
                                      fwvalid_reg |
                                      // Keep high till the transfer has completely left the channel
                                      (responder_quiescentn_q & (fw_chan_cactive | fw_reg_cactive))
                                     );
  ...
  wire responder_quiescent_and_stalledn_nxt    = (// Drive high because of a new transfer
                                                  ...
                                                  responder_fence_n |
                                                  // Keep high till the transfer has completely left the channel
                                                  (responder_quiescent_and_stalledn_q & (fw_chan_cactive | fw_reg_cactive))
                                                 );
  ...
  //wire                            fwvalid_reg;
  wire                            fwready_reg;
  wire [((FW_PAYLOAD_WIDTH>0)?(FW_PAYLOAD_WIDTH-1):0):0]     fw_payld_reg;

  wire fwvalid_chan;
  assign fwvalid_reg = (fwvalid_chan & inner_fence_n);

  assign wakeup_local = (fwvalid_reg | fwvalid_int);
  ...
  u_dnstrm_fw
    (
      ...
      .dst_valid (fwvalid_chan),
      .dst_ready (fwready_reg),
      .dst_payld (fw_payld_reg),
  ...
  generate
    if ((FW_OPREG==0) || (FW_PAYLOAD_WIDTH==0))
      begin : g_no_fw_opreg
        assign fwvalid_int = fwvalid_reg;
        assign fwready_reg = fwready_int;
        assign fw_payld    = fw_payld_reg;

        assign fw_reg_cactive = 1'b0;
      end
    else
      begin : g_fw_opreg
        assign fw_reg_cactive = fwvalid_int;

        adb400_r3_ful_regd_slice
          #(.PAYLD_WIDTH(FW_PAYLOAD_WIDTH))
        u_fw_opreg
          (
           .aclk      (aclk),
           .aresetn   (resetn_forced),
           .valid_src (fwvalid_reg),
           .ready_src (fwready_reg),
           .payld_src (fw_payld_reg),
           .valid_dst (fwvalid_int),
           .ready_dst (fwready_int),
           .payld_dst (fw_payld)
          );
      end
  endgenerate
  ...
  u_upstrm_rv
    (
      ...
      .src_valid (rvvalid_int),
      .src_ready (rvready_int),
      .src_payld (rv_payld),
